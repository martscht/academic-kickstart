...
Das Paket `GGally` stellt die praktische Funktion `ggpairs()` bereit. Wir illustrieren dies am R-internen Datensatz `iris`. Der Datensatz enthält für drei Arten von Iris-Blumen (Variable `Species`) vier Variablen mit Messungen von Blütenblättern:  

```{r}
str(iris)
```

Die Verteilungen von sowie Zummenhänge zwischen den verschiedenen Messwerten lassen sich mit `ggpairs()` illustrieren: 
...
Richtig spannend wird es aber erst, wenn wir zusätzlich noch separate Verteilungen für jede der drei Blumenarten (`Species`) anfordern: 
