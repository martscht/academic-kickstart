...
<details><summary>**Inferenzstatistik der Partialkorrelation und weiter konzeptionelle Überlegungen**</summary>
...
Wir sehen also, dass das Residuum einen Mittelwert von 0 hat und unkorreliert vom Prädiktor ist. Nun interessiert uns aber, wie stark dieses Residuum noch mit der abhängigen Variable (*x* oder *y*) zusammenhängt:
...
Interessierten sei gesagt, dass die Formel für `r_xy.z` das Gleiche ist wie: 

`r_xy.z <- (r_xy - r_xz*r_yz)/sqrt(1-r_xz^2)/sqrt(1-r_yz^2)`

mit dem Vorteil, dass man die Klammer im Nenner nicht vergessen kann.
...
Super, auch diesmal stimmen die Formeln. Wir sehen also, dass die `cor.test` Funktion gar nicht zum richtigen Ergebnis kommen *kann*, wenn wir ihr die Residuen übergeben. Das liegt daran, dass die `cor.test` Funktion davon ausgeht, dass ganz normale Variablen korreliert werden sollen. Residuen sind aber dahingehend besonders, dass sie bereits verrechnet wurden. Wir haben ja bereits Regressionen mit *z* vorgenommen, um die entsprechenden Anteile von *z* herauszurechnen. Damit haben wir bereits Informationen über die Daten benutzt. Nämlich diese, die die Korrelationen mit *x* und *y* mit *z* betreffen. Diese genutzen Informationen müssen eingepreist werden, was durch das Anpassen der Freiheitsgrade in der $t$-Verteilung geschieht (wir nutzen hier `df = n-1-2` und nicht `df=n-2`). Im Laufe Ihres Studiums  wird dieser Sachverhalt immer wieder aufgegriffen und auch noch näher erklärt. An dieser Stelle reicht es uns zu wissen, dass wir bei der Partialkorrelation andere $df$ verwenden müssen.
