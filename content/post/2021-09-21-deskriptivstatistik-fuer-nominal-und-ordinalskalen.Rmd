...
Nachdem wir in der letzten Sitzung die Arbeit mit dem CSV Format kennen gelernt haben, nutzen wir jetzt die Datei im RDA Format für das Einlesen des Datensatzes. Die Datei können Sie [hier <i class="fas fa-download"></i> herunterladen](/post/fb21.rda). Sie können ihn aber alternativ auch mit dem folgenden Befehl direkt aus dem Internet einladen.
...
In der letzten Sitzung haben wir schon einige Befehle für das Screening eines Datensatzes kennen gelernt. Dabei zeigt `names` alle Variablennamen an, während `dim` uns Zeilen und Spalten ausgibt. Der Datensatz hat also `r nrow(fb21)` Beobachtungen auf `r ncol(fb21)` Variablen.
...
```{r, eval = FALSE}
jpeg("Mein-Barplot.jpg", width=15, height=10, units="cm", res=150)
barplot(tab,
 col = rainbow(5),
 ylab = 'Anzahl Studierende',
 main = 'Lieblingsfach im 1. Semester',
...
```{r}
- 1/log(dim(table(fb21$fach))) * sum(prop.table(table(fb21$fach)) * log(prop.table(table(fb21$fach))))
```
...
In diesem Beispiel betragen Median und Q1 jeweils 2, sodass sich die entsprechenden Linien überlagern. Ein  Beispiel für einen "schöneren" Boxplot (ohne Überlagerung) ist dieses:
