...
```{r}
table(fb21$mdbf4)               # Absolute Haeufigkeiten
prop.table(table(fb21$mdbf4))   # Relative Haeufigkeiten
which.max(table(fb21$mdbf4))    # Modus
```
...
```{r}
median(fb21$mdbf4)                 # Ohne Argument für NA: funktioniert nicht
median(fb21$mdbf4, na.rm = TRUE)   # Expliziter Ausschluss: funktioniert
```

Der Median für die Variable `mdbf4` beträgt also `r median(fb21$mdbf4, na.rm = TRUE)`.
...
```{r}
quantile(fb21$mdbf4, .75, na.rm=T) - quantile(fb21$mdbf4, .25, na.rm=T)
```

Der IQB der Variable `mdbf4` beträgt `r quantile(fb21$mdbf4, .75, na.rm=T) - quantile(fb21$mdbf4, .25, na.rm=T)`.
