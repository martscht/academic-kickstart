...
```{r}
table(fb21$prok1)               # Absolute Haeufigkeiten
prop.table(table(fb21$prok1))   # Relative Haeufigkeiten
which.max(table(fb21$prok1))    # Modus
```
...
```{r}
median(fb21$prok1)                 # Ohne Argument für NA: funktioniert nicht
median(fb21$prok1, na.rm = TRUE)   # Expliziter Ausschluss: funktioniert
```

Der Median für die Variable `prok1` beträgt also `r median(fb21$prok1, na.rm = TRUE)`.
...
```{r}
quantile(fb21$prok1, .75, na.rm=T) - quantile(fb21$prok1, .25, na.rm=T)
```

Der IQB der Variable `prok1` beträgt `r quantile(fb21$prok1, .75, na.rm=T) - quantile(fb21$prok1, .25, na.rm=T)`.
