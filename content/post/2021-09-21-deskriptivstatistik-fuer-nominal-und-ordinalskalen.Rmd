...
Den Datensatz können Sie [hier <i class="fas fa-download"></i> herunterladen](/post/fb21.rda). Sie können ihn aber alternativ auch mit dem folgenden Befehl direkt aus dem Internet einladen.
...
Der Datensatz hat also `r nrow(fb21)` Beobachtungen auf `r ncol(fb21)` Variablen.
...
```{r}
fb21$geschl_faktor <- factor(fb21$geschl,                                   # Ausgangsvariable
                             levels = 1:3,                                  # Faktorstufen
                             labels = c("weiblich", "männlich", "anderes")) # Label für Faktorstufen
str(fb21$geschl_faktor)
head(fb21$geschl_faktor)
```
...
```{r}
str(fb21$grund)                            # Ursprungsvariable: Character
fb21$grund_faktor <- as.factor(fb21$grund) # Umwandlung in Faktor
str(fb21$grund_faktor)                     # neue Variable: Faktor
```


Die neue Variable ist nun ein Faktor mit `r length(levels(fb21$grund_faktor))` Stufen. Das Vorgehen ist nur zur Anschauung gedacht und in diesem speziellen Fall nicht sinnvoll, da jede einzelne Freitextantwort vermutlich nur genau einmal vorkommt und später sowieso nicht (ohne zusätzliche Kodierung) in statistischen Analysen weiterverwendet werden kann.
...
```{r}
levels(fb21$fach)         # Abruf

fb21$fach <- relevel(
  fb21$fach,              # Bezugskategorie wechseln
...
```{r}
tab <- table(fb21$fach) # Absolute Haeufigkeiten
sum(tab)                # Gesamtzahl
...
Die Darstellung als Tabelle wirkt häufig langweilig. Zu viele Tabellen in einem Bericht / einer Arbeit schrecken Leser:innen meist ab. Nachfolgend werden mögliche grafische Darstellungsformen für diskrete nominalskalierte Variablen gezeigt.
...
```{r}
relinf <- (ln_hj * hj) |> sum() * (-1/log(5))  # Relativer Informationsgehalt
relinf
```
...
```{r}
- 1/log(5) * sum(prop.table(table(fb21$fach)) * log(prop.table(table(fb21$fach))))
```

Der relative Informationsgehalt der Variable `fach` beträgt demnach `r round(relinf, 3)`. Da der mögliche Wertebereich zwischen 0 (für alle Personen selbe Kategorie) und 1 (für alle Kategorien gleich viele Personen) variiert, kann hier von einer starken Verteilung der Personen ausgegangen werden.
...
Es treten die Werte `r min(fb21$prok4,na.rm=T)` bis  `r max(fb21$prok4,na.rm=T)` empirisch auf, außerdem gibt es  `r sum(is.na(fb21$prok4))` fehlende Werte (dargestellt als `NA`):
...
```{r}
table(fb21$prok4)               # Absolute Haeufigkeiten
prop.table(table(fb21$prok4))   # Relative Haeufigkeiten
which.max(table(fb21$prok4))    # Modus
```
...
```{r}
median(fb21$prok4)                 # Ohne Argument für NA: funktioniert nicht
median(fb21$prok4, na.rm = TRUE)   # Expliziter Ausschluss: funktioniert
```

Der Median für die Variable `prok4` beträgt also `r median(fb21$prok4, na.rm = TRUE)`.
...
```{r}
quantile(fb21$prok4, .75, na.rm=T) - quantile(fb21$prok4, .25, na.rm=T)
```

Der IQB der Variable `prok4` beträgt `r quantile(fb21$prok4, .75, na.rm=T) - quantile(fb21$prok4, .25, na.rm=T)`.
