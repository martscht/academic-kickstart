...
<!-- Vielleicht können wir hier nochmal ein bisschen Struktur reinbringen, sodass es nicht wirkt wie eine Aneinaderreihung von Funktionen. Zumindest die ersten drei Funktionien sind ja auch eng verwandt - vielleicht nochmal einen zweiten Absatz in dem einführenden Text hier einfügen, was einen erwarten wird und dann gerne auch mit klickbaren Links - schau dir nochmal an, wie das geht, in smartgit ist deine Appendix Verlinkung nicht klickbar, weil der Verweis fehlt -->
...
Jetzt wollen wir diesen `Commit` rückgängig machen. 
...
<!-- Habe jetzt halt noch das SmartGit Commit eingefügt, oder willst du das per Terminal?-->
...
Wenn ihr den Befehl `$git reset --hard [Commit]` in eurem <span style="color: darkred;">**Git**</span>-Repository ausführt, wird der Verlauf auf diesen ausgewählten `Commit` zurückgesetzt. Die ungewollten `Commits` kann man im Log nicht mehr sehen (wie es oben der Fall ist). Das Log könnt ihr euch mit `git log` anzeigen lassen, mit "q" (quit) könnt ihr es wieder verlassen. es sieht so aus: 

<!-- Das Bild ist ja eher der Grund, warum man reset nutzen sollte in manchen Fällen - würde ich vielleicht eher nach oben ziehen mit Beschreibung und dadurch den Vorteil nochmal klarer machen -->
...
<!-- Das heißt, wenn ich zwei oder drei Commits rückgängig machen wollen würde, müsste ich das öfter klicken? Gibt es da keine andere Möglichkeit? -->
