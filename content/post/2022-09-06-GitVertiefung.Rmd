...
<!-- Was machen die jeweiligen Befehle getrennt - also warum braucht man zwei davon? Weiterhin: kann man sie vielleicht anders formatieren, um klar zu machen, dass es Terminal Code ist? Zur Not einfach ein Screenshot -->
...
Das gibt uns nun die Möglichkeit, doch noch mit dem ungewollten `Commit` arbeiten zu können, falls wir unsere Meinung ändern sollten. Dieser wird nämlich nicht komplett aus dem Log entfernt 

<!-- Kann sein, dass du dafür die Screenshots nochmal neu machen müsstest, aber kannst du den gesamten Ablauf der Commits auch nochmal darstellen? -->
...
<!-- Nochmal aufführen, warum das sinnvoller sein könnte und deutlich machen, dass wir hier bei Git sind -->
<!-- Bitte generell mehr Screenshots, wo man was genau sehen kann - also wo steht das im Log bspw -->
...
<!-- Das könnte besser zur Erklärung am Start des Abschnitts dann passen -->
...
<!-- Kannst du die Datei vielleicht etwas kleiner oder irgendwie anders schärfer machen? -->

Hier klickt man auf *Stage*, um den Konflikt zu lösen. *Stage* bereitet die Datei für einen `Commit` vor, lädt sie also ins *Staging Environment* wie wir aus dem **Git-Intro** wissen. Würden wir auf *Don´t Stage* klicken, hätten wir zwar unsere Veränderungen in der Datei, aber sie würden beim nächsten `Commit` nicht auftauchen. Wichtig: Anschließend muss noch ein `Commit` und ein `Push` durchgeführt werden, um die neue Datei auch auf <span style="color: darkred;">**GitHub**</span> zu haben. Ansonsten droht euch ein größerer Konflikt, wenn jemand anderes noch weiter an derselben Datei arbeitet. Nach dem Push wurde der Konflikt erfolgreich gelöst.

<!-- Die Konsequenz von dont stage verstehe ich ehrlich gesagt noch nicht. Würde das dann auch gar nicht in der Files Ansicht zum Auswählen von einem neuen Konflikt erscheinen? -->


# Fazit und Ausblick

Wie ihr sicherlich gesehen habt, sind <span style="color: darkred;">**Git**</span> und <span style="color: darkred;">**SmartGit**</span> umfangreicher als es auf den ersten Blick scheint. Denn auch dieses Kapitel hat das ganze Ausmaß nur angerissen. Falls ihr euch mal einen Überblick über alle Funktionen von <span style="color: darkred;">**Git**</span> schaffen wollt, schaut in die offizielle <span style="color: darkred;">**Git**</span>-Dokumentation rein. 
