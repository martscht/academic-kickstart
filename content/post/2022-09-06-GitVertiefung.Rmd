...
In <span style="color: darkred;">**SmartGit**</span> funktioniert es recht einfach. Ihr klickt auf *Local*, oben in der Leiste und dann auf `Undo last Commit`. Wenn ihr mehrere `Commits` rückgängig machen wollt, müsst ihr einfach so oft auf `Undo last Commit` bis ihr an eurem "Wunsch"-`Commit` angekommen seid. 

![](/post/smartgit_UndoLastCommit1.png)

Anschließend müsst ihr das noch bestätigen und kehrt dann auf den Stand vor diesem `Commit` zurück.

![](/post/smartgit_UndoLastCommit2.png)

<!-- Das heißt, wenn ich zwei oder drei Commits rückgängig machen wollen würde, müsste ich das öfter klicken? Ja genau. -->
<!-- Das klingt umständlich - schau da bitte nochmal nach. Und wenn es so ist, würde ich es im Text zumindest mal erwähnen -->
<!-- Alle Tutorials, die ich gefunden habe bzgl. mehrere Commits rückgängig machen, erklären das nur in Git. zu SmartGit gibt es zu dem Thema quasi nichts. -->
...
<!-- Hier ist ein Minus und später kommen zwei Minus bei git -reset. Ist das dann der Unterschied? Oder wie kommt das zustande?-->
