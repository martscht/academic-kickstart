...
```{r, echo=FALSE}
deskr <- describeBy(fb21$lz, fb21$ort)
fra <- summary(fb21[which(fb21$ort=="Frankfurt"), "lz"])
and <- summary(fb21[which(fb21$ort=="anderer"), "lz"])
```
...
```{r, echo=FALSE}
wilcox <- wilcox.test(fb21$lz ~ fb21$ort, paired = F, alternative = "less",  conf.level = .95)
```

**Formales Berichten des Ergebnisses** 

Es wurde untersucht, ob außerhalb von Frankfurt wohnende Studierende zufriedener im Leben sind als die in Frankfurt wohnenden. Deskriptiv  zeigt sich tatsächlich, dass die Nicht-Frankfurter:innen zufriedener sind (_Mdn_ = `r round(deskr$anderer$median, 1)`, _IQB_ = [`r round(and[2], 1)` ; `r round(and[5], 1)`]) als die Frankfurter:innen (_Mdn_ = `r round(deskr$Frankfurt$median, 1)`, _IQB_ = [`r round(fra[2], 1)` ; `r round(fra[5], 1)`]). entsprechende Wilcoxon-Test zeigt jedoch ein nicht signifikantes Ergebnis (_W_ = `round(wilcox$statistic, 1)`, _p_ = `r round(wilcox$p.value, 3)`). Die Nullhypothese konnte nicht verworfen werden und wird beibehalten. Die Studierenden sind gleich zufrieden, unabhängig von ihrem Wohnort.
