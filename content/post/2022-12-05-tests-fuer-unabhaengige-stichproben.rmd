...
subtitle: ''
summary: ''
authors: [koehler, buchholz, irmer, goldhammer]
...
4.  Fragestellung D: Sind jüngere Geschwister kooperativer als ältere? (*t-Test und Cohen's d für abhängige Stichproben*)
5.  Fragestellung E: Sind jüngere Geschwister kooperativer als ältere? (*Wilcoxon-Test für abhängige Stichproben*)
...
### 1.1 Vorbereitung (Daten einlesen, Daten aufbereiten)
...
dataB <- fb22[ (fb22$geschl=="maennlich"|fb22$geschl=="weiblich"), ]  
...
```{r}
# Gruppierter Boxplot :
boxplot(dataB$nerd ~ dataB$geschl, 
...
```{r}
# umständlich:
summary(dataB$nerd[(dataB$geschl=="weiblich")])  # Gruppe 1: Studentinnen
summary(dataB$nerd[(dataB$geschl=="maennlich")]) # Gruppe 2: Studenten

# komfortabler:
library(psych)
describeBy(x = dataB$nerd, group = dataB$geschl)        # beide Gruppen im Vergleich 
```
...
```{r}
nerd.weiblich <- dataB$nerd[(dataB$geschl=="weiblich")]
...
nerd.maennlich <- dataB$nerd[(dataB$geschl=="maennlich")]
sigma.maennlich <- sd(nerd.maennlich, na.rm = T)
n.maennlich <- length(nerd.maennlich[!is.na(nerd.maennlich)])
sd.maennlich <- sigma.maennlich * sqrt((n.maennlich-1) / n.maennlich)
sd.maennlich
```
...
# Gruppe 1 (weiblich) 
par(mfrow=c(1,2))
nerd.weiblich <- dataB[(dataB$geschl=="weiblich"), "nerd"]
...
# Gruppe 2 (maennlich)
par(mfrow=c(1,2))
nerd.maennlich <- dataB[(dataB$geschl=="maennlich"), "nerd"]
hist(nerd.maennlich, xlim=c(0,6), ylim=c(0,.8), main="Nerdiness Testwert (maennlich)", xlab="", ylab="", las=1, prob=T)
curve(dnorm(x, mean=mean(nerd.maennlich, na.rm=T), sd=sd(nerd.maennlich, na.rm=T)), col="blue", lwd=2, add=T)
qqnorm(nerd.maennlich)
qqline(nerd.maennlich, col="blue")
```

$\rightarrow$ Normalverteilung kann auch für Gruppe 2 (maennlich) einigermaßen angenommen werden (hierbei ist zu beachten, dass die Gruppe der Studenten im Vergleich zur Gruppe der Studentinnen deutlich kleiner ist)
...
```{r}
library(car)
leveneTest(dataB$nerd ~ dataB$geschl)
```

```{r, echo=FALSE}
levene <- leveneTest(dataB$nerd ~ dataB$geschl)
...
-   $H_0$: $\mu_\text{weiblich} = \mu_\text{maennlich}$ bzw. $\mu_\text{weiblich} - \mu_\text{maennlich} = 0$\
-   $H_1$: $\mu_\text{weiblich} \ne \mu_\text{maennlich}$ bzw. $\mu_\text{weiblich} - \mu_\text{maennlich} \ne 0$
...
```{r}
t.test(dataB$nerd ~ dataB$geschl,       # abhängige Variable ~ unabhängige Variable
...
```{r, echo=FALSE}
ttest <- t.test(dataB$nerd ~ dataB$geschl, paired = FALSE, alternative = "two.sided", var.equal = TRUE, conf.level = .95)
```
...
nerd.maennlich <- dataB[(dataB$geschl=="maennlich"), "nerd"]
mw.maennlich <- mean(nerd.maennlich, na.rm=T)
n.maennlich <- length(nerd.maennlich[!is.na(nerd.maennlich)])
sigma.qu.maennlich <- var(nerd.maennlich, na.rm=T) 

sigma.inn <- sqrt((sigma.qu.weiblich* (n.weiblich - 1) + sigma.qu.maennlich* (n.maennlich - 1)) / (n.weiblich-1 + n.maennlich-1))

d1 <- (mw.weiblich - mw.maennlich) / sigma.inn
d1
```
...
```{r}
d2 <- cohen.d(dataB$nerd, dataB$geschl, na.rm=T)
d2
```
...
Es wurde untersucht, ob sich Studentinnen und Studenten in dem Nerdiness-Testwert unterscheiden. Tatsächlich findet sich deskriptiv ein Unterschied: Studentinnen weisen einen durchschnittlichen Wert von `r round(mw.weiblich,3)` (*SD* = `r round(sd.weiblich,3)`) auf, während die Studenten einen Wert von `r round(mw.maennlich,3)` (*SD* = `r round(sd.maennlich,3)`) aufweisen. Dies entspricht nach Cohens Konvention (1988) einem kleinen bis mittleren Effekt (*d* = `r round(d1,3)`).
...
```{r}
# Gruppierter Boxplot:
boxplot(dataB$nr2 ~ dataB$geschl, 
...
```{r}
describeBy(dataB$nr2, dataB$geschl) # beide Gruppen im Vergleich

# Interquartilsbereich (IBQ) über summary()
summary( dataB[(dataB$geschl=="weiblich"), "nr2"])
summary( dataB[(dataB$geschl=="maennlich"), "nr2"]) 
```

```{r, echo=FALSE}
deskr <- describeBy(dataB$nr2, dataB$geschl)

# Interquartilsbereich (IBQ) über summary()
weibl <- summary( dataB[(dataB$geschl=="weiblich"), "nr2"])
maennl <- summary( dataB[(dataB$geschl=="maennlich"), "nr2"]) 
```
...
nr2.w <- dataB[(dataB$geschl=="weiblich"), "nr2"]
hist(nr2.w, xlim=c(0,6), ylim=c(0,.6), main="Item nr2 (weiblich)", xlab="", ylab="", las=1, prob=T, breaks = c(.5:5.5))

nr2.m <- dataB[(dataB$geschl=="maennlich"), "nr2"]
hist(nr2.m, xlim=c(0,6), ylim=c(0,.6), main="Item nr2 (maennlich)", xlab="", ylab="", las=1, prob=T, breaks = c(.5:5.5))
...
```{r}
library(car)
leveneTest(dataB$nr2 ~ dataB$geschl)
```

```{r, echo=FALSE}
levene2 <- leveneTest(dataB$nr2 ~ dataB$geschl)
...
```{r}
wilcox.test(dataB$nr2 ~ dataB$geschl,     # abhängige Variable ~ unabhängige Variable
            paired = FALSE,               # Stichproben sind unabhängig
...
```{r, echo=FALSE}
wilcox <- wilcox.test(dataB$nr2 ~ dataB$geschl, paired = FALSE, alternative = "greater", conf.level = .95)
```
