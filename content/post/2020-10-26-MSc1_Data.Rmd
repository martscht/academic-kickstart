...
summary: ''
authors: [irmer]
lastmod: '2020-11-03T10:58:21+02:00'
featured: no
header:
...
Hier wird noch für Sie gebaut `r fontawesome::fa("tools")`. Die Übung wird vorbereitet. Haben Sie noch etwas Geduld.

## Sitzung 4: 

Wir wollen uns einen simulierten Datenbeispiel zu Einstellungsentscheidungen in jungen Start-Ups ansehen (Datensatz `Assessment` aus dem gleichnamigen .rda File `Assessment.rda`). Sie können den   [`r fontawesome::fa("download")` Datensatz "Assessment.rda" hier herunterladen](/post/Assessment.rda) und diese anschließend mit `load` laden (bspw. auf dem Desktop von Frau "Musterfrau").
...
Außerdem können Sie den Datensatz selbst wie folgt selbst kürzen. Dazu müssen Sie nur das *.zip*-File herunterladen und das Working Directory entsprechend legen, sodass Sie die Daten einlesen können. Wir verwenden nur Daten aus Deutschland, Frankreich und den Vereinigten Arabischen Emiraten. 
