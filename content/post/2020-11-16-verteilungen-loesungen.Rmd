...
Zum Abschluss werden wir auch hier eine Variable aus unserem Datensatz betrachten. Dabei wird es um Neurotizismus gehen.
...
```
Einladen können wir den Datensatz über den gewohnten Befehl. Da wir Werte wie den Mittelwert für die Überprüfung der Normalverteilung verwenden wären, ist es nötig, eine numerische Variable vorliegen zu haben (`is.numeric`). Das Resultat `TRUE` zeigt uns an, dass dies gegeben ist. 
...
```{r}
qqnorm(fb22$neuro)
qqline(fb22$neuro, col = "red")
```

Der QQ-Plot zeigt ein ähnliches Muster. Die Auftretenspunkte der Stichprobe sind früher, als sie nach der theoretischen Erwartung sein sollten. Trotzdem ist auch hier die Verletzung auf wenige punkte fokussiert, weshalb wir keine starke Verletzung annehmen.

Die Färbung erhalten wir in diesem Fall dadurch, dass wir in `qqline` noch eine Farbe im Argument `col` ergänzen.
