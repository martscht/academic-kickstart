...
Die einzelnen Vektoren gehören alle zur Klasse `numeric`, da sie nur Zahlen beinhalten. Die Dimensionen des Datensatzes zu den `lz_items` betragen `r nrow(lz_items)` Zeilen und `r ncol(lz_items)` Spalten (in dem Fall die Anzahl der Lebenszufriedenheites-Items).
...
* Der Modus der Variable `farbe` ist blau und kommt `r max(table(data$farbe))` mal vor.
...
mean(data$lz_ges)
#Varianz (beide Wege)
var(data$lz_ges) * (11 / 12)
sum((data$lz_ges - mean(data$lz_ges))^2) / 12
```

* Das arithmetische Mittel beträgt `r round(mean(data$lz_ges), digits = 3)`.
* Je nach Rechenweg beträgt die Varianz `r round(var(data$lz_ges) * (11 / 12), digits = 3)` oder `r round(sum((data$lz_ges - mean(data$lz_ges))^2) / 12, digits = 3)`.
...
plot(fb21$lerntyp, main = "Lerntypen Jahrgang 2021", ylab = "Anzahl Studierende", col = colours)
plot(fb21$lerntyp, main = "Lerntypen Jahrgang 2021", ylab = "Anzahl Studierende", col = colours2)
``` 
</details>
...
Vertrauenswürdigkeit ist in `vertr` abgelegt. 
...
Nun können Mittelwerte für die beiden Gruppen bestimmt werden. 

```{r}
mean(fb21_alleine$extra)
mean(fb21_gruppe$extra)

mean(fb21_alleine$nerd)
mean(fb21_gruppe$nerd)
```
