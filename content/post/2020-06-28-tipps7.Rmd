...
<details><summary>Abschnitt anzeigen</summary>
Wenn man die Abbildung aus Projekt 5 übernimmt, hat man ein Sudoku-Gitter mit farbig hinterlegten Zahlen und der Überschrift "Sudoku". Als Abbildung allein ergibt das vielleicht Sinn, doch für die App sollten daran einige Veränderungen vorgenommen werden:
...
Da erst auf Knopfdruck eine Zahl an die gewählte Position gesetzt werden soll, nutzt man nun eine `observeEvent`-Funktion, um das reaktive Objekt (z.B. `rv$data`) um die einzusetzende Zahl an der gewählten Position zu erweitern. Diese Zahl soll jedoch nur dann an der gewählten Position eingesetzt werden können, wenn an der gewählten Position im Ausgangssudoku (z.B. `BeispielSudoku`) keine vorgegebene Zahl steht. Das kann man mithilfe einer `if`-Funktion implizieren, da über den gesamten Lösungsprozess hinweg das Ausgangssudoku (`BeispielSudoku`) nicht verändert wird und deshalb als Objekt für den Abgleich fungieren kann.
...
#### [Weiter zur Lösung](/post/loesungen7)
