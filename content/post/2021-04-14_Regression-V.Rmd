...
lastmod: '2022-05-29T08:32:21+02:00'
featured: no
header:
...
knitr::purl("2021-04-14_Regression-V.Rmd", documentation = 0) #Legt fest das Purl nur die Chunkinhalte(0) in ein R-Skript umwandelt!; NAME DES RMD MUSS EINGESTELLT WERDEN!
file.rename("2021-04-14_Regression-V.R","PsyBSc7_R_Files/12_reg5.R") #NAME DER R-DATEI MUSS ANGEPASST WERDEN AN RMD-NAMEN!UNTERORDNER SOLLTE MIT KURS ÜBEREINSTIMMEN! (Siehe Ordner in /post)
```
...
Bisher hatten wir mittels Regressionsanalysen lineare Beziehungen modelliert. In der Sitzung zur [quadratischen und moderierte Regresssion](/post/quadratische-und-moderierte-regression) kamen dann im Grunde quadratische Effekte mit hinzu. Wir können unser Wissen über Regressionen allerdings auch nutzen um nichtlineare Effekte zu modellieren. Wie das geht und was zu beachten ist, schauen wir uns im Folgenden an. Dazu laden wir zunächst altbekannte Pakete:
...
<details><summary>**Wie gut kennen Sie sich noch mit Exponenten und Logarithmen aus?**</summary>
...
<details><summary>**Wie gut kennen Sie sich noch mit Exponenten und Logarithmen aus? ---  Antworten**</summary>

In fett, kursiv und mit einem `r fontawesome::fa("smile")` werden Ihnen die richtigen Antworten angezeigt: 

* **_Die Gleichung $10^x = 100$ können wir bspw. mit dem 10er-Logarithmus $log_{10}(100)$ berechnen. Die Antwort ist 2._** `r fontawesome::fa("smile")`
* Die Gleichung $10^x = 100$ können wir bspw. mit dem 10er-Logarithmus $log_{10}(100)$ berechnen. Die Antwort ist 3.
* Logarithmen können NICHT in einander umgerechnet werden.
* Es gilt: $log(a+b)=log(a)log(b)$.
* **_Es gilt: $log(ab)=log(a)+log(b)$._** `r fontawesome::fa("smile")`
* **_$log_a(x)$ ist die Umkehrfunktion zu $a^x$. Dies bedeutet, dass $log_a(a^x)=x.$_** `r fontawesome::fa("smile")`
* Es gilt: $a^{c+d}=a^c+a^d$.
* **_Es gilt: $a^{c+d}=a^ca^d$._** `r fontawesome::fa("smile")`
* Es gilt immer $a^x < b^x$ für a < b und alle x.
* Es gilt immer $a^x < b^x$ für 1 < a < b und alle x.
* **_Es gilt immer $a^x < b^x$ für 0 < a < b und alle x > 0._** `r fontawesome::fa("smile")`
* Es gilt immer $a^x > b^x$ für 0 < a < b < 1 und alle x.
* Es gilt immer $a^x < b^x$ für 0 < a < b < 1 und alle x.
* **_Es gilt immer $a^x > b^x$ für 0 < a < b und alle x < 0._** `r fontawesome::fa("smile")`
</details>
...
### Exkurs
<details><summary>**Exponenten und Logarithmen: ein paar Rechenregeln zur Wiederholung**</summary>
...
```{r, fig.height=4, fig.align="center", echo=-1}
# Lineares Modell für das Bevölkerungswachstum
...
$$
sodass wir einen neuen Interzept (verschoben um $\beta_2$) und einen neuen Effekt des Jahres (verändert um $\beta_3$) ab diesem Punkt haben. In `R` können wir das Ganze genauso ausdrücken, wie wir es für [andere Regressionen mit Interaktionen](/post/quadratische-und-moderierte-regression) schon gesehen hatten:
...
## R-Skript
Den gesamten `R`-Code, der in dieser Sitzung genutzt wird, können Sie [`r fontawesome::fa("download")` hier herunterladen](/post/PsyBSc7_R_Files/12_reg5.R).
...
<details><summary>**Quadratisches Modell für das Bevölkerungswachstum**</summary>
...
```{r exercise_graph_pop_quad-solution, purl=FALSE}
ggplot(data = WorldPopulation, aes(x = Year, y = Population))+
     geom_point()+geom_smooth(method="lm", formula = "y~x")+         # plotte linearen Verlauf 
...
<details><summary>**Quadratisch-exponentielles Modell für das Bevölkerungswachstum**</summary>
...
```{r exercise_log_Pop_reg_quad-solution, purl=FALSE}
...
```{r resid_log_Pop_reg_quad, purl=FALSE}
...
```{r excercise_log_Pop_reg_predict_quad-solution, purl=FALSE}
...
```{r final_pred_comp, fig.height=6, fig.align="center", purl=FALSE}
ggplot(data = WorldPopulation, aes(x = Year, y = Population))+
     geom_point()+geom_smooth(method="lm", formula = "y~x")+         # plotte linearen Verlauf 
     geom_line(aes(x = Year, y = pred_Pop_exp), col = "red", lwd = 1.5) +
...
<details><summary>**Übersicht über erklärte Varianzanteile**</summary>
...
<details><summary>**Übersicht über die Modelle**</summary>
