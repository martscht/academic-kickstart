...
```
Wenn du dir nun den Datensatz anschaust sollte er so aussehen
...
Falls du `ggplot` noch nicht geladen hast, solltest du es jetzt tun.
```{r}
library('ggplot2')
```

Als erstes kannst du dir über die Funktion `summary` einen groben Überblick über Mittelwerte und Standardabweichungen des Datensatzes machen.
```{r, eval=F}
summary(whatsapp)
```

Unser erstes Diagramm ist ein Kreisdiagramm, dazu wer am häufigsten schreibt. Die brauchst hier den Befehl `table` damit du die Häufigkeiten der Personen bekommst.
```{r}
pie(table(whatsapp$author), col = c("red", "yellow", "green", "violet", "orange", "blue", "pink", "cyan") )
```
...
```{r}
hist(whatsapp$time, frequency(whatsapp$text), breaks = 20)
...
ggplot(whatsapp, aes(x = whatsapp$uhrzeit))+ geom_histogram(bins = 10, color = 'white') +
  xlab('Zeit')+ ylab('Nachrichten') + geom_freqpoly()
```
...
```{r}
hans <- whatsapp[grep('Hans', whatsapp$author), 'text']
hans <- paste(as.character(hans), collapse = ' ')
```
