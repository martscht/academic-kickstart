...
```{r, echo=FALSE}
levene <- leveneTest(dataA$Punktzahl ~ dataA$Gruppe)
f <- round(levene$`F value`[1], 2)
p <- round(levene$`Pr(>F)`[1], 3)
```

_F_(`r levene$Df[1]`,`r levene$Df[2]`) = `r f`, _p_ = `r p` $\rightarrow$ Das Ergebnis ist nicht signifikant, die $H_0$ wird beibehalten. Homoskedastizität wird angenommen.
...
```{r, echo=FALSE}
ttest <- t.test(dataA$Punktzahl ~ dataA$Gruppe, paired = FALSE, alternative = "two.sided", var.equal = TRUE, conf.level = .95)
```
_t_(_df_ = `r ttest$parameter`, zweis.) = `r round(ttest$statistic, 3)`, _p_ < .001 $\rightarrow$ signifikant, $H_0$ wird verworfen, $H_1$ wird angenommen.  
...
Zur Beantwortung der Fragestellung wurde ein _t_-Test unter Annahme von Homoskedastizität (_F_(`r levene$Df[1]`,`r levene$Df[2]`) = `r f`, _p_ = `r p`) durchgeführt. Der Gruppenunterschied ist signifikant (_t_(_df_ = `r ttest$parameter`, zweis.) = `r round(ttest$statistic, 3)`, _p_ < .001), somit wird die Nullhypothese verworfen und die Alternativhypothese angenommen. Studierende, die regelmäßig am Tutorium teilgenommen haben, unterscheiden sich im Ergebnis der Statistikklausur (erreichte Punktzahl) bedeutsam von jenen, die nur unregelmäßig da waren.
...
dataB$geschl <- droplevels(dataB$geschl) # levels aus den Datensatz entfernen, die keine Erhebungen haben
...
```
```{r, echo=FALSE}
deskr <- describeBy(dataB$vertr, dataB$geschl)

# Interquartilsbereich (IBQ) über summary()
weibl <- summary( dataB[(dataB$geschl=="weiblich"), "vertr"])
maennl <- summary( dataB[(dataB$geschl=="maennlich"), "vertr"]) 
```
...
```{r, echo=FALSE}
levene2 <- leveneTest(dataB$vertr ~ dataB$geschl)
...
```{r, echo=FALSE}
wilcox <- wilcox.test(dataB$vertr ~ dataB$geschl, paired = FALSE, alternative = "greater", conf.level = .95)
```
...
Es wurde untersucht, ob Studentinnen verträglicher sind als Studenten. Deskriptiv besteht kein Unterschied in der Verträglichkeit: Beide Gruppen weisen einen Median von `r round(deskr$weiblich$median, 2)` auf ($IQB_w$ = [`r round(weibl[2], 2)`; `r round(weibl[5], 2)`]; $IQB_m$ = [`r round(maennl[2], 2)`; `r round(maennl[5], 2)`]). Zur Beantwortung der Fragestellung wurde ein Wilcoxon-Test durchgeführt. Der Gruppenunterschied über das Geschlecht ist nicht statistisch bedeutsam (_W_ = `r wilcox$statistic`, _p_ = `r round(wilcox$p.value, 3)`). Somit wird die Nullhypothese beibehalten: Studentinnen sind nicht verträglicher als Studenten. 
