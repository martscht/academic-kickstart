...
Laden Sie die Datensatz ein. Diesen haben Sie bereits aus [<i class="fas fa-download"></i> `fb21.rda`](/post/fb21.rda) heruntergeladen oder durch den unten aufgeführten direkten Befehl eingeladen. Die Bedeutung der einzelnen Variablen und ihre Antwortkategorien können Sie weiterhin dem Dokument [Variablenübersicht.pdf](/post/variablen.pdf) entnehmen.
...
naturverbundenheit <- fb21[, c('nr1', 'nr2', 'nr3', 'nr4', 'nr5',  'nr6')]
fb21$nr_ges <- rowMeans(naturverbundenheit)
```
...
fb21$nr_ges <-  fb21[, c('nr1', 'nr2', 'nr3', 'nr4', 'nr5',  'nr6')] |> rowMeans()
```
...
median(fb21$nr_ges, na.rm = TRUE)
mean(fb21$nr_ges, na.rm = TRUE)
...
Zur Berechnung der Varianz gemäß Formel benötigen wir $n$. Wir könnten mit `nrow(fb21)` die Länge des Datensatzes für `n` heranziehen. Dies ist jedoch nur dann sinnvoll, wenn auf der Variable `nr_ges` keine fehlenden Werte vorhanden sind!

```{r}
is.na(fb21$nr_ges) |> sum()
```

Hier gibt es tatäschlich wieder zwei fehlenden Werte. Im Tutorial haben wir aber bereits gelernt, dass man mit `length(na.omit(fb21$nr_ges))` die Anzahl an Personen bestimmen kann, die auf der Skala einen Wert haben.

```{r}
# empirische Varianz
# per Hand
sum((fb21$nr_ges - mean(fb21$nr_ges, na.rm = T))^2, na.rm = T) / (length(na.omit(fb21$nr_ges)))

# durch Umrechnung 
var(fb21$nr_ges, na.rm = T) * (length(na.omit(fb21$nr_ges))-1) / length(na.omit(fb21$nr_ges))

# Populationsschätzer
var(fb21$nr_ges, na.rm = T)
```
...
(sum((fb21$nr_ges - mean(fb21$nr_ges, na.rm = T))^2, na.rm = T) / length(na.omit(fb21$nr_ges))) |> sqrt()

# Populationsschätzer
sd(fb21$nr_ges, na.rm = T)
```
...
# z-Standardisierung
fb21$nr_ges_z <- scale(fb21$nr_ges)
...
par(mfrow=c(1,2))
hist(fb21$nr_ges_z, breaks = 20)
hist(fb21$nr_ges, breaks = 20)
```
