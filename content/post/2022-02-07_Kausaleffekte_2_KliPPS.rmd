...
summary: ''
authors: [hartig]
lastmod: '2022-02-07 17:00:00 CEST'
featured: no
header:
...
Wir verwenden wieder unserer fiktives Datenbeispiel, in dem Patient\*innen, die an einer Depression oder einer Angststörung leiden, entweder mit einer kognitiven Verhaltenstherapie (CBT) behandelt oder in einer Wartekontrollgruppe belassen wurden. Die Zuordnung konnte nicht randomisiert erfolgen, weshalb die Gruppen nicht vergleichbar sind. 
...
Matching können wir komfortabel mit der Funktion `matchit` aus dem Paket `MatchIt` durchführen. Mit Standard-Einstellungen wird als Methode zur Paarbildung ein "greedy nearest neighbor matching" verwendet, bei dem jedem Fall aus der Treatment-Gruppe nacheinander der ähnlichste Fall der Kontrollgruppe zugeordnet wird. Hier ist es wichtig, die Option `caliper` zu nutzen, um den zulässigen Mindestabstand zwischen den "Zwillingen" zu definieren (ausgedrückt in SD-Einheiten). Die folgende Grafik demonstriert den Effekt des caliper auf die Balance des Propensity Scores.

```{r include=FALSE, eval = F}
i <- 1;
for(c in c(NA, 1, 0.5, 0.3, 0.1)) {
  if(is.na(c)) {
    PS.match <- matchit(Treatment ~ Disorder + BDI_pre + SWL_pre, method = "nearest",
                        data = CBTdata, link = "logit")
  } else {
    PS.match <- matchit(Treatment ~ Disorder + BDI_pre + SWL_pre, method = "nearest",
                        data = CBTdata, link = "logit", caliper = c)
  }
  CBTdata.PSM <- match.data(PS.match)
  CBTdata.PSM$PS_logit <- psych::logit(CBTdata.PSM$distance)
  CBTdata.PSM$caliper <- c
  CBTdata.PSM$frame <- i
  if(is.na(c)) { CBTdata.PSM$caliper <- "NULL" }
  if(i==1) {c.data <- CBTdata.PSM} 
  else { c.data <- rbind(c.data, CBTdata.PSM) } 
  i <- i + 1
}

c.data <- c.data[order(c.data$frame),]

library(gganimate)
anim <- ggplot(c.data, aes(x=PS_logit, fill = Treatment)) + 
  theme_bw() +
  scale_fill_manual(values=c("#E69F00", "#56B4E9")) +
  geom_density(alpha=0.3) +
  transition_states(frame, transition_length = 1, state_length = 1)

c.text <- c(1, 0.5, 0.3, 0.1, "NULL (default!)")

anim <- anim + labs(title = '{paste0("caliper = ", c.text[as.integer(closest_state)] )}')

if(file.exists("MatchIt.Caliper.gif")) {file.remove("MatchIt.Caliper.gif")}
anim_save(filename = "MatchIt.Caliper.gif", animation = anim)
```

![*Abbildung: PS-Score-Dichte mit unterschiedlichen Werten für den caliper*](MatchIt.Caliper.gif)
 
### Kontrolle der Kovariaten-Balance

Mit dem kleinsten hier verwendeten Wert von 0.1 SD wird eine gute Balance des Propensity Score erreicht, diesen verwenden wir für die weiteren Analysen:
...
Stratifizierung ist als Methode `subclass` in der `matchit`-Funktion enthalten. Wir bilden fünf Strata und extrahieren den Datensatz, der die Zugehörigkeit zu den Strata enthält (Variable `subclass`):
...
Den Treatment-Effekt berechnen wir hier jetzt "per Hand", indem wir die Effekte innerhalb der Strata berechnen und dann über diese eine nach Größe der Strata gewichtete Summe berechnen. Hier erhalten wir mit `r round(ATT.strat,2)`einen geringfügig geringeren Effekt als bei anderen Methoden.
