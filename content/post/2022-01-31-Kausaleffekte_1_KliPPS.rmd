...
lastmod: '2022-02-02 12:51:12 CEST'
featured: no
header:
...
beschreibt die bedingten Effekte bedingt auf `Disorder`. Hier wird also im Grunde der Haupteffekt der `Disorder` untersucht. `E[g1(K,Z)|K=0]` beschreibt hierbei den $ATE|$`ANX` und `E[g1(K,Z)|K=1]` den $ATE|$`DEP`. Patient\*innen mit Angstsymptomatik zeigten also eine deskritiv größere Verbesserung durch das `Treatment`.
...
Unter der Stable Unit Treatment Value Assumption (SUTVA) und der Strong Ignoribility Annahme bedeutem die Ergebnisse also, dass es einen Effekt der CBT gab (mit einer Irrtumswahrscheinlich von 5%) und dass dieser Effekt für unterschiedliche Symptomatiken unterschiedlich stark ausgeprägt war.
