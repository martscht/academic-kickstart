...
Wir sehen, die Koeffizienten liegen recht nah an den "wahren" Werten. Verpacken wir das Ganze in eine Funktion, so können wir den Bias der Schätzung untersuchen. Der Bias ist die durchschnittliche Abweichung der Schätzung vom wahren Wert. Ein Bias von 0 ist somit erstrebenswert!
...
<details><summary>**`while` und `repeat`-Loops & weitere Loop-Funktionen**</summary>
...
Auch `apply` und seine Varianten können genutzt werden, um bspw. eine `for`-Loop auszudrücken. Diese Funktion verkürzt die Schreibweise und kann manchmal auch die Laufzeit verkürzen, insbesondere wenn bspw. das `pbapply`-Paket verwendet wird, welches einfaches Parallelisieren erlaubt. 
