...
subtitle: ''
summary: ''
authors: [cezanne, lawetzky, schultze]
...
*Dieses Tutorial wird aktuell weiter ergänzt.*

In diesem Tutorial soll gezeigt werden, wie Sie mit Hilfe von FormR Fragebögen erstellen können. Das Tutorial soll Sie dabei speziell auf die Anforderungen vorbereiten, die sich Ihnen im Rahmen Ihrer Gruppenarbeit stellen werden. Wir präsentieren die Funktionsweise von FormR anhand einer abgewandelten und auch verkürzten Version des Fragebogens, welchen Sie in der ersten Woche im Praktikum selbst ausgefüllt haben.
...
Zudem benötigt der FormR-Account Adminrechte, um Fragebögen zu erstellen. Dafür müssen Sie eine Email an die Adresse accounts@formr.org schreiben, in der Sie Adminrechte für Ihren Account anfragen. In der Regel sollten sie eine Antwort erhalten, sobald Sie die Adminrechte erhalten haben. Sie können das auch daran erkennen, dass wenn Sie sich bei FormR einloggen, in der oberen Leiste ein Feld mit "Admin" steht.
...
Fragebögen lassen sich bei formR über Spreadsheets erstellen. Sie müssen also eine Tabelle erstellen, die formR dann in einen Fragebogen umwandelt. Die Tabelle können Sie mit einem Tabellenprogramm, wie Excel, Gnumeric oder auch mit Ihrem etwaigen Google Account über Google Sheets erstellen.

Ihre Tabelle sollte folgende Spalten besitzen:
...
Die Spalte `explanantion` ist für Ordnung und das eigene Verständnis zuständig. Hier können Sie eigene Notizen hinterlassen, die für die Teilnehmenden nicht sichtbar sind. Bei der Erhebung invertierter Items ist zu empfehlen dies dort zu vermerken, damit diese bei der Auswertung nicht übersehen werden.
...
*Bild Zustimmung Seite 1*

Mit "mc_width70" (bzw. 50, ... ,100, 150, 200) kann bei multiple choice items der Abstand zwischen den Auswahlmöglichkeiten eingestellt werden und mit "hide_label mc_width70" (bzw. 50, ... ,100, 150, 200) werden die `labels` der Auswahlmöglichkeiten nicht angezeigt. So werden die Auswahlmöglichkeiten hier nur in der Überschrift angezeigt und nicht auch neben den Auswahlkästchen.

*Bild Seite 2 und Excel*

Mit `type` wird die Art des Items festgelegt. Für die Datenschutzerklärung brauchen wir hierbei „note" und „check". Mit "note" wird ein Text dargestellt, den wir in die Spalte `label` schreiben. In der Spalte Label werden Textelemente geschrieben, die für die Teilnehmenden sichtbar sein sollen. Zudem müssen die Teilnehmenden der Erklärung zustimmen. Dafür nutzen wir den `type` „check", der eine Box erstellt, die zur Bestätigung angeklickt werden kann.

*Bild von Excel und dem fertigen Fragebogen Seite 1*
...
Ratingbuttons lassen sich mit dem `type` „rating_button (Min, Max, Step)", wobei Min, Max und Step für das niedrigste Rating, das höchste Rating und die Schrittweite stehen.

Sollen die Teilnehmenden frei antworten können, ist dies mit "text" möglich.

*Bild Grund für Psychostudium*

Weitere mögliche Items finden sie [hier](https://formr.org/public/documentation#available_items)

In der Spalte `optional` können Sie mit `*` deutlich machen, dass das Item nicht beantworten werden muss.

In der Spalte `name` wird den Variablen ein Name gegeben. Diese sind für die Teilnehmenden nicht sichtbar. Es handelt sich um die Variablennamen mit denen Sie bei R die Auswertung durchführen.

Mit `Show if` kann angegeben werden, dass ein Item nur unter einer bestimmten Bedingung angezeigt werden soll.

Mit `choice1`, `choice2`, ... werden die Bezeichnungen für die Antwortmöglichkeiten angegeben. Für die single choice Frage zum Geschlecht sind die Antwortmöglichkeiten hier „männlich", „weiblich" und „anderes":

*Bild des Geschlechtsitems*
...
**5. Schritt VPM Umfrage hochladen**

Um Versuchspersonenminuten zu vergeben, müssen wir auf die gleiche Weise wie eben eine weitere Umfrage erstellen. Die Vorlage für die Erhebung der Namen und Email-Adressen, die wir für die Ausstellung der VPM benötigen, sind im [Moodle Kurs](https://moodle.studiumdigitale.uni-frankfurt.de/moodle/pluginfile.php/419733/mod_folder/content/0/vpm.ods?forcedownload=1) zu finden. Dieses Sheet müssen wir herunterladen und lokal abspeichern und auf formR eine weitere Umfrage mit diesem Dokument erstellen.

**6. Schritt einen Run erstellen**

Für unsere fertige Studie, müssen wir nun einen `Run` erstellen, indem wir unseren Fragebogen mit der VPM-Abfrage verknüpfen. Einen `Run` erstellen wir, indem wir in der Adminansicht auf `Create Run` drücken.
...
Um die AnonymitÃ¤t der Teilnehmenden zu gewÃ¤hrleisten, setzen wir in den Einstellungen das Häkchen bei `Unlink Survey` und speichern die Änderung:
...
**9. Schritt Daten herunter laden**
...
Der Nachteil dieses Vorgehens ist, dass wir uns auf eine statische Fassung der Daten einlassen müssen. Besonders wenn Sie bereits während der Erhebung damit beginnen wollen, Auswertungsskripte zu verfassen kann es aber besser sein, die Daten direkt in R von FormR herunterzuladen. Der Prozess der hier beschrieben wird, wird z.B. auch in der Statistikvorlesung für die Folien mit Beispielen genutzt, die direkt während der Sitzung erhoben werden.
...
Die Daten der VPM-Umfrage speichern wir nach Abschluss der Umfrage unter einem geigneten Namen ab (z.B "Extraversion und Bildschirmzeit - Schultze.csv") und senden sie per Mail an h.rouchi@psych.uni-frankfurt.de.
...
![](/post/formrfaq-unlink-survey.jpg)
