...
-	Explanation: wird den TN nicht angezeigt; dient für Notizen
-	Label: Text für TN
- Class: visuelle Formatierung
- Type: Antwortart (Likert-Skala = rating_button)
- Optional: mit „*“ kann angegeben werden, dass TN Frage überspringen dürfen
- Name: Variablenname (darf sich nicht wiederholen)
- Showif: Filter, um bestimmte Fragen unter bestimmten Voraussetzungen anzuzeigen
- Choice1, choice2, ...: verschiedenen Antwortmöglichkeiten definieren

Zunächst sollten Sie alle Spalten richtig benennen und zuordnen können. Die Spalte `explanantion` ist für Ordnung und das eigene Verständnis zuständig. Bei der Erhebung invertierter Items ist zu empfehlen, die Invertierung in die Anmerkungen in der Spalte `explanation` zu schreiben, damit diese bei der Auswertung nicht übersehen werden.
 In der Spalte `optional` können Sie mit `*` deutlich machen, dass das Item nicht beantworten werden muss. Bei der Erhebung invertierter Items ist zu empfehlen, die Invertierung in die Anmerkungen in der Spalte `explanation` zu schreiben, damit Sie dies nicht bei der Auswertung übersehen.
