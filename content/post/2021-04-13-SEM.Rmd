---
title: Gerichtete Beziehungen mit Pfadanalysen und Strukturgleichungsmodelle
author: ''
date: '2021-04-13'
slug: sem
categories: 
...
In dieser Sitzung beschäftigen wir uns mit Pfadanalysen und Strukturgleichungsmodellen (engl. **S**tructural **E**quation **M**odeling, SEM). Diese werden beispielsweise in [Werner, Schermelleh-Engel, Gerhard und Gäde (2016, Kapitel 17 in Döring & Bortz, 2016)](https://hds.hebis.de/ubffm/Record/HEB36808809X) oder [Eid, Gollwitzer und Schmitt (2017)](https://hds.hebis.de/ubffm/Record/HEB366849158) in Kapitel 26 ausführlich beschrieben.
...
In dieser Sitzung erweitern wir dafür unsere Kenntnisse mit dem `R`-Paket `lavaan` um gerichtete Abhängigkeiten. Möchten Sie die Grundlagen im Umgang damit wiederholen, so empfiehlt es sich die erste Sitzung nochmals anzusehen ([Einführungssitzung zu `lavaan`](/post/lavaan-intro)). Auch baut diese Sitzung auf der vergangenen [Sitzung zur CFA](/post/cfa) auf.
...
Der totale Effekt von Zeitdruck auf die psychosomatischen Beschwerden ergibt sich als $TE:=IE + DE = ab+c$. Da wir bisher in `lavaan` die Mittelwertstruktur nicht mitmodelliert hatten, haben wir $a_0$ und $c_0$ auch noch nicht untersucht. Für weitere inhaltliche Informationen zu Pfadanalysen, direkten, indirekten und totalen Effekten lesen Sie gerne in [Werner et al. (2016, pp. 952-)](https://hds.hebis.de/ubffm/Record/HEB36808809X) oder [Eid et al. (2017, pp. 952-)](https://hds.hebis.de/ubffm/Record/HEB366849158). _Man bemerke den unfassbaren Zufall, dass beide Bücher an der gleichen Seitenzahl beginnend über Mediationen etc. sprechen!_

In `lavaan` Koeffizienten zu benennen, ist ganz einfach. Sie haben es vielleicht schon im Appendix der letzten Sitzung zur [CFA](/post/cfa) gesehen: der Variable wird der Koeffizientenname gefolgt von dem Multiplikationszeichen `*` vorangestellt. Also wird die Beziehung zwischen `BOEEs` und `ZDs` um das Präfix `a*` ergänzt zu: `BOEEs ~ a*ZDs`, usw.:
...
```{r, echo = F}
load(url("https://pandar.netlify.app/post/fit_paths_IE_TE_boot.RData"))
cat("[...]")
...
_Wem dieses Konzept bekannt vorkommt, hat sich wahrscheinlich an die Unterscheidung zwischen [EFA](/post/efa) und [PCA](/post/PCA) zurückerinnert: Hier war es so, dass bei der PCA die Hauptkomponenten Linearkombinationen, also gewichtete Summen, der beobachteten Items waren, während bei EFA ein Messmodell mit dahinterliegenden latenten Variablen formuliert wurde und somit Messfehler modeliert werden konnten._
...
Wer davon noch nicht genug hat, ist herzlich eingeladen im [Appendix A](#AppendixA) sich die mit dem Test auf essentielle $\tau$-Äquivalenz vertraut machen. Die Annahme der essentiellen $\tau$-Äquivalent wurde in der [Sitzung zur CFA](/post/cfa) bereits angesprochen unterliegt implizit dem Zusammenfassen zu Skalenmittelwert. 

Den gesamten R-Code, der in dieser Sitzung genutzt wird, können Sie [`r fontawesome::fa("download")` hier herunterladen](https://raw.githubusercontent.com/jpirmer/MSc1_FEI/master/R-Scripts/SEM.R).

***

## Appendix A {#AppendixA}

<details><summary>**Essentielle $\tau$-Äquivalenz im SEM**</summary>
...
Auf die gleiche Art und Weise können wir auch Invarianz (zu Invarianz über Gruppen siehe [Sitzung zur MSA](/post/msa)) über die Zeit oder über Gruppen hinweg untersuchen. Über die Zeit würden wir zwei latente Variablen formulieren, welche einmal $t_1$ und einmal $t_2$ symbolisiert. Die Annahme, dass zu beiden Zeitpunkte die Messmodelle gleich sind, ist tatsächlich in vielen Situation sinnvoll.
...
[Werner, C. S., Schermelleh-Engel, K., Gerhard, C., & Gäde, J. C. (2016).](https://hds.hebis.de/ubffm/Record/HEB36808809X) Strukturgleichungsmodelle. In N. Döring & J. Bortz, *Forschungsmethoden und Evaluation (5., vollständig überarbeitete und erweiterte Auflage, pp. 945--975)*. Berlin: Springer.
