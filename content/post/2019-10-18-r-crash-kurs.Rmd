...
summary: ''
authors: [schultze]
lastmod: '2023-02-03T16:20:00+01:00'
featured: no
header:
...
```{css, echo=FALSE}
/* CSS um Ankerverhalten zu optimieren (Header ist im Weg) */
.anchorheader {
    scroll-margin-top: 3rem; /* Sorgt für vertikale Distanz wo der fixed Header ist */
}
.anchor {
    scroll-margin-top: 4rem; /* Größer weil inline vermutlich kleinere Eigenmargin hat                                  und weil es schön auf die wichtigen Inhalte zoomt */
    
    display:inline-block; /* Notwendig, damit einzelne Anker die selbe Funktion nutzen                               können */ 
}
```
...
* [Was ist die **Konsole** und was ist die **Syntax**?](#Konsole_Syntax)
...
<a class="anchor" id="Konsole_Syntax"></a>

RStudio besteht aus vier Panels. Zunächst sind nur drei sichtbar - durch **Strg+Shift+n** (OS X: **Cmd+Shift+n**) oder über den ![](/post/2019-10-18-r-crash-kurs_files/new_script.png) Button öffnen Sie eine neue Skriptdatei und das vierte Panel erscheint. <a class="anchor" id="Eingabe"></a>
R ist syntaxbasiert - genau genommen ist R eigentlich eine Programmiersprache und kein Auswertungsprogramm - und im neu erschienenen Fenster können Sie diese Syntax schreiben. Das pure schreiben bewirkt zunächst nichts. <a class="anchor" id="Ergebnisse"></a>
Damit etwas passiert, muss die Syntax mit **Strg+Return** (OS X: **cmd+Return**) oder mit dem ![](/post/2019-10-18-r-crash-kurs_files/run.png) Button ausgeführt werden. Wenn Sie z.B. `3 + 4` in die Syntax schreiben und so ausführen, erscheint in der Konsole:
...
<a class="anchor" id="Taschenrechner"></a>
