...
summary: ''
authors: [schultze]
lastmod: '2020-10-24T10:35:06+01:00'
featured: no
header:
...
<details><summary>Kernfragen dieses Beitrags</summary>
...
Wenn wir das Ergebnis unseres zweiten Schritts (also wenn wir die Wurzel ziehen), direkt wieder einem Objekt zuzuweisen, passiert Folgendes:
...
Die Daten aus der Befragung, die Sie letzte Woche ausgefüllt haben finden Sie [<i class="fas fa-download"></i> hier](/post/fb21.csv). Diese liegen im CSV Format vor und die Datei heißt **fb21.csv**. Mit `read.table` können wir diesen Datensatz laden, müssen aber bestimmte Eigenheiten des Datensatzes bedenken. Wenn Sie den Datensatz mit einem Text-Editor öffnen sehen die ersten 5 Zeilen folgendermaßen aus:

```{r, echo = FALSE}
cat(paste(readLines('fb21.csv')[1:5], collapse = '\n'))
```
...
```{r}
fb21 <- read.table('fb21.csv', header = TRUE)
```
...
```{r}
fb21 <- read.table('fb21.csv', header = TRUE, sep = ",")
```

Im Environment erscheint jetzt das Objekt `fb21`. Mit `head` können wir uns den Kopf des Datensatzes (die ersten 6 Zeilen) anzeigen lassen:

```{r}
head(fb21)    # Kopfzeilen
str(fb21)     # Struktur des Datensatzes
```
...
```{r}
write.table(fb21,     # zu speichernder Datensatz
  'fb21.txt'          # Dateiname
  )
```
...
```{r, echo = FALSE}
cat(paste(readLines('fb21.txt')[1:5], collapse = '\n'))
```
