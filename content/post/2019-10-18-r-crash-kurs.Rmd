...
RStudio besteht aus vier Panels. Zunächst sind nur drei sichtbar - durch **Strg+Shift+n** (OS X: **Cmd+Shift+n**) oder über den `r blogdown::shortcode("inline_image", "/post/2019-10-18-r-crash-kurs_files/new_script")` Button öffnen Sie eine neue Skriptdatei und das vierte Panel erscheint. `r blogdown::shortcode("intext_anchor", "Eingabe")`
...
Damit etwas passiert, muss die Syntax mit **Strg+Return** (OS X: **cmd+Return**) oder mit dem `r blogdown::shortcode("inline_image", "/post/2019-10-18-r-crash-kurs_files/run")` Button ausgeführt werden. Wenn Sie z.B. `3 + 4` in die Syntax schreiben und so ausführen, erscheint in der Konsole:
