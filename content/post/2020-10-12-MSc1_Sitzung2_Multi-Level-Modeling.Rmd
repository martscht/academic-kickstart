...
summary: ''
authors: [irmer]
lastmod: '2020-10-27T17:32:21+02:00'
featured: no
header:
...
Um zu testen, ob die Varianz des Motivationseffekts signifikant wird, wird das Modell `m1` ohne *random slope* mit dem Modell `m2` mit *random slope* verglichen. Dies machen wir mit Hilfe der `anova` Funktion, welche in `R` dazu genutzt wird, um Modelle zu vergleichen. Hierbei ist es essentiell, dass wir zuvor das Paket `lmerTest` geladen haben, da es sonst zu fehlerhaften Signifikanztests kommen kann.  Der `anova`-Funktion müssen wir zunächst das restriktivere Modell (mit weniger $df$) übergeben und als zweites Argument das weniger restriktive Modell. Außerdem wählen wir explizit die Methode, mit welcher die Modelle verglichen werden sollen via `test = "LRT"`. Somit verwenden wir den Likelihood-Ratio-Test: Dieser hat ohne das zusätzliche Laden des `lmerTest`-Pakets allerdings das Problem, dass der $\alpha$-Fehler zu groß sein kann, wenn wir Varianzen  auf Signifikanz prüfen. Dies liegt daran, dass Varianzen nicht negativ sein können, wir daher eine gerichtete Hypothese formulieren müssen und daher den $p$-Wert eines Varianzvergleichs in MLM mittels LRT durch zwei teilen müssen. Dies gilt allerdings ausschließlich für Varianzen (vgl Eid, et al., 2017, Seite 749 und folgend). Da aber automatisch mit der Random Slope auch die Kovarianz zwischen Interzept und Slope in `lmer` hinzugefügt wird, stimmt auch das wiederum nicht. _Machen wir uns es daher einfach und verwenden das `lmerTest`-Paket!_ Das Laden des Pakets `lmerTest` benutzt eine Misch-$\chi^2$-Verteilung, um diesem Problem aus dem Weg zu gehen. Allerdings müssen wir hierzu zusätzlich noch `refit = F` wählen und so der `anova`-Funktion zu sagen, dass REML-Schätzungen (und damit die Korrektur der Verteilungen wegen des Testens der Varianzen) verwendet werden sollen. Außerdem könnten Sie auch die `ranova` Funktion des `lmerTest`-Pakets nutzen, um alle Random-Effekte eines Modells zu prüfen. Sehen Sie selbst, beide Tests kommen zu selben Ergebnis (für über diesen Kurs hinausgehende Literatur zu diesem Thema siehe bspw. dem dazu erschienen [`r fontawesome::fa("graduation-cap")` Artikel](https://www.jstatsoft.org/article/view/v082i13)). 
...
Diese Informationen sollten nicht inhaltlich interpretiert werden, sondern geben Auskunft über die Schätzung der Parameter (wenn REML verwendet wird, wird in einem zweistufigen Verfahren zunächst die zufälligen Effekte geschätzt und dann werden die festen Effekte geschätzt und eine Signifikanzentscheidung ist für die festen Effekte möglich). Die Schätzung des Interzepts korreliert sehr hoch negativ mit der Schätzung des Koeffizienten der Klassengröße, was daran liegen kann, dass die Klassengröße keinerlei Vorhersagekraft liefert.
