...
```{r}
#Zunächst Missings aus dem Datensatz identifizieren und neuen Datensatz ohne Missings erstellen
missings <- which(apply(confirmed_raw[,1:4],1,function(x) any(is.na(x)))) 
confirmed <- confirmed_raw[-missings,]  
```
...
```{r}
#Zunächst Missings aus dem Datensatz identifizieren und neuen Datensatz ohne Missings erstellen
missings <- which(apply(deaths_raw[,1:4],1,function(x) any(is.na(x)))) 
deaths <- deaths_raw[-missings,]  
```
