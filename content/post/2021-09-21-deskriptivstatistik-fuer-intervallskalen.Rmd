...
Den Datensatz haben wir bereits über diesen Link heruntergeladen [hier <i class="fas fa-download"></i> herunterladen](/post/fb21.rda)
...
Vor dem Start des Skripts muss er natürlich wieder in unser Environment eingeladen werden.
...
Achtung! Dies funktioniert in unserem Fall wieder nicht, da die Verwendung von `nrow(fb21)`  - wie oben bereits angemerkt - nicht sinnvoll ist: `nrow(fb21)` ist nicht gleich n (eine Person hat einen fehlenden Wert), daher besser:
...
```{r}
# Umrechnung der Varianzen
var(fb21$lz, na.rm = TRUE) * (96 - 1) / 96
```
...
Wir müssen das Ergebnis also wieder mit einem Faktor multiplizieren, um die emprische Standardabweichung zu erhalten. Alternativ kann diese natürlich auch komplett händisch berechnet werden.

```{r}
# Händische Berechnung der empirischen Standardabweichung
sqrt(sum((fb21$lz - mean(fb21$lz, na.rm = TRUE))^2,
         na.rm = TRUE) / (length(na.omit(fb21$lz))))
```
...
**Skalenwert Prokrastinationstendenz**
