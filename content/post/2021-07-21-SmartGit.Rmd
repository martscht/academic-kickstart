...
<span style="color: darkred;">**SmartGit**</span> ist wie bereits geschildert eine grafische Oberfläche für die Verwendung von <span style="color: darkred;">**Git**</span> auf dem lokalen Rechner - aber auch in der Verbindung mit **remote repositories**. Das Programm existiert bereits seit 2009 und wurde stets weiterentwickelt, wodurch eine reibungslose Funktionsweise sichergestellt ist. Durch Ausführen von <span style="color: darkred;">**Git**</span>-Befehlen via Klick wird die Arbeit beschleunigt. Andere Befehle werden sogar automatisiert durchgeführt. Ein großer Vorteil von <span style="color: darkred;">**SmartGit**</span> ist dabei, dass es mit mehreren Orten für **remote repositories** wie <span style="color: darkred;">**GitHub**</span>, <span style="color: darkred;">**Bitbucket**</span> oder <span style="color: darkred;">**GitLab**</span> integrativ arbeiten kann, wobei wir uns im Tutorial weiterhin nur mit <span style="color: darkred;">**GitHub**</span> beschäftigen werden.
...
Jetzt wo wir <span style="color: darkred;">**SmartGit**</span> erfolgreich installiert und mit unserem Praktikums-Ordner verknüpft haben, stellt sich die Frage, wofür und wie wir <span style="color: darkred;">**SmartGit**</span> am besten verwenden. Hierzu betrachten wir zunächst, wo wir unsere bereits erlernten <span style="color: darkred;">**Git**</span>-Befehle umsetzen können
...
Diese solltet ihr nun kopieren und wieder zurück auf <span style="color: darkred;">**SmartGit**</span> wechseln. Dort wählte ihr in der Übersichtsleiste zunächst *Remote* und dann *Add...*. Es erscheint eine Eingabe für die URL, die ihr eben kopiert habt.  
...
Mit dem Befehl `git revert HEAD` wird ein neuer `Commit` erstellt, der den vorherigen reverted. Wenn ihr `git log` ausführt, werdet ihr eueren ungewollten `Commit` zwar noch sehen, aber rein technisch ist dieser rückgängig gemacht worden. In unserer Datenauswertung haben wir jetzt etwas Falsches reingeschrieben. 

![](/post/smartgit_revert.png)
Gehen wir nun über das Terminal in unser Repository und führen den obigen Befehl aus. Wenn wir jetzt unser Log checken, sollte da sowas stehen: 
...
Der Conflict Solver ist keine Funktion. Um diesen benutzen zu können muss man erstmal einen Konflikt haben. 

Konflikte können entstehen, wenn zwei Personen diesselbe Zeile derselben Datei ändern oder eine Person die Datei löscht, während ein andere diese ändert. 

Das verwirrt <span style="color: darkred;">**Git**</span>, da es nicht weiß, welche der beiden Änderungen nun richtig ist und zeigt einen Konflikt an. Jetzt muss der Entwickler, der als letztes seine Änderungen gepushed hat, den Konflikt lösen. Mit <span style="color: darkred;">**SmartGit**</span> geht das zum Glück relativ leicht. 

**Konflikt erstellen**

Um einen Konflikt lösen zu können, muss man natürlich erstmal einen erstellen. Alleine geht das wie folgt: 

1. Erstellt eine neue Datei in R, die ihr dann committed und auf <span style="color: darkred;">**GitHub**</span> pusht, sodass ihr lokal und remote denselben Stand habt. 
2. Jetzt ändert ihr lokal etwas in der Datei (hier in Zeile 20).

![](/post/smartgit_KonfliktLokal2.png)
3. Öffnet <span style="color: darkred;">**SmartGit**</span> und führt nur einen `Commit` durch, danach nicht wie normalerweise noch einen `Push`.  
4. Wechselt nun zu <span style="color: darkred;">**GitHub**</span>  und ändert dort etwas in derselben Zeile, in der ihr auch lokal etwas verändert habt (hier also Zeile 20). Jetzt speichert ihr diese Änderungen.  
5. Wechselt wieder zu <span style="color: darkred;">**SmartGit**</span> und führt einen `Pull` durch. 

Wenn ihr alle diese Schritte durchgeführt habt, ist ein Konflikt entstanden. 
...
Dabei entscheidet man sich entweder komplett für seine eigene Version oder die des anderen Entwicklers. 

Das ist natürlich problematisch, wenn man sowohl die Eigenen als auch die des Anderen behalten will. Hier kommt jetzt der `Conflict Solver` ins Spiel. Den Button dazu findet ihr in dem Bereich unten rechts.  
