...
Das standardisierte Regressionsgewicht der Lebenszufriedenheit $\beta_\text{Lebenszufriedenheit}$ liegt bei `r round(coef(lm.beta(model))[3], 4)`, was bedeutet, dass bei einer Erhöhung der Lebenszufriedenheit um eine Standardabweichung die Depressivitaet im Mittel (im Durchschnitt) um `r round(coef(lm.beta(model))[3], 4)` Standardabweichungen fällt. Da das Geschlecht *hier* nur zwei Ausprägungen hat, lässt sich der standardisierte Koeffizient $\beta_\text{Geschlecht}$ von `r round(coef(lm.beta(model))[2], 4)` wie folgt interpretieren: Steigt die Variable Geschlecht um eine Standardabweichung - werden also Frauen im Vergleich zu Männern betrachtet - so steigt die Depressivitaet um durchschnittlich `r round(coef(lm.beta(model))[2], 4)` Standardabweichungen. 
...
Jetzt nochmal zurück zum zweiten Einsatz der Werte - der Prüfung der multivariaten Normalverteilung. Die Verteilung der Mahalanobisdistanz widerspricht nicht (zu sehr) der Annahme auf multivariate Normalverteilung, da das Histogramm einigermaßen zur Dichte der $\chi^2(df=2)$-Verteilung passt:
