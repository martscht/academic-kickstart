...
```{r}
t.test(x = dataMeditation[, "Vorher"], y  = dataMeditation[, "Nachher"], # die beiden abhängigen Gruppen
...
```{r, echo=FALSE}
ttest <- t.test(x = dataMeditation[, "Vorher"], y  = dataMeditation[, "Nachher"], paired = T, alternative = "less", conf.level = .95)
```

* Zur Erinnerung: _df_ bei _t_-test mit abhängigen Stichproben: _n_ - 1 (wobei _n_ die Anzahl der Paare darstellt)
* _t_(0.05;`r ttest$parameter`) = `r round(ttest$statistic, 3)`, _p_ < .05 $\rightarrow$ signifikant, H0 wird verworfen, H1 wird angenommen.
...
$\Rightarrow$ Der standardisierte Populationseffekt beträgt _d2''_ = `r round(d_Meditation, 2)` und ist laut Konventionen nach Cohen (1988) groß. 
...
Zur Beantwortung der Fragestellung wurde ein gerichteter _t_-Test für abhängige Stichproben durchgeführt. Der Gruppenunterschied ist signifikant (_t_(`r ttest$parameter`) = `r round(ttest$statistic, 3)`, _p_ < .05), somit wird die Nullhypothese verworfen und die Alternativhypothese angenommen. Die Meditation hat einen Einfluss auf die Zufriedenheit.

Dieser Einfluss ist nach dem standardisierte Populationseffekt von _d''_ = `r round(d_Meditation, 2)` groß.
