...
Falls du von alleine nicht weiterkommen solltest, kannst du dir die **Tipps** anschauen. Tipp 1 hilft dir dabei, bei der Datenaufbereitung die Zeilenumbrüche im Datensatz zu beheben:
...
In Tipp 2 wird erläutert, wie du die ausgeschlossenen Medien behandeln kannst. 


<details><summary>Tipp 2</summary>

Die ausgeschlossenen Medien lassen sich identifizieren, indem mit `grep` die Einträge eines character-Vektors ermittelt werden, die einen bestimmten Ausdruck enthalten. 
...
Für unseren Freundetest erstellen wir nun eine neue Tabelle, in der wir Punkte vergeben. In dieser Tabelle müssen die Namen aller Teilnehmer und das Ergebnis der Zeitanalyse stehen. Für die Punktevergaben gibt es einen einfachen Befehl, versuche ihn über die Hilfefunktion zu finden.
...
Das Folgende wurde alles bereits erklärt, der Vollständigkeit halber steht es hier aber nochmals. Um `udpipe` benutzen zu können, musst du das Modell in der richtigen Sprache herunterladen und in die aktuelle Sitzung laden.
```{r, fig = F} 
# model <- udpipe_download_model(language = 'german')
