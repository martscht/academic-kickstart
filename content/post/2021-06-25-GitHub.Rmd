...
subtitle: ''
summary: ''
authors: [nehler, rouchi, schloter]
featured: no
header:
  image: "/header/github.jpg"
...
Dadurch weiß <span style="color: darkred;">**Git**</span> jetzt, an welchem Ort das Repository online gelagert wird. Um jetzt die Inhalte, die wir lokal schon haben, auch online angezeigt zu bekommen, muss noch ein `push` durchgeführt werden. Diesen Begriff werden wir später nochmal genauer erläutern - wir führen ihn hier aber schonmal aus. `master` ist hierbei der Name eines Branches. Das Prinzip von Branches lernen wir erst in einem späteren [Tutorial](/post/branches) kennen. Wir erwähnen es an dieser Stelle aber, da  <span style="color: darkred;">**Git**</span> und assoziierte Programme an einer sensitiveren Sprache arbeiten (`master` soll daraus aufgrund des Bezugs zur Sklaverei verbannt werden). Es kann also sein, dass euer Branch nicht `master` sondern `main` heißt. Herausfinden könnt ihr das durch den bekannten Befehl:

```
git status
```

Hier wird euch der Name angezeigt. Danach könnt ihr den passenden der beiden folgenden Befehle auswählen.
...
![](/post/git_github_connect_first2.png)

Im Terminal oder durch ein Pop-Up wird man nach der Registrierung von <span style="color: darkred;">**GitHub**</span> gefragt, wodurch sicher gestellt wird, dass niemand Fremdes einfach ein Repository mit seinen Dateien überschreibt. Je nach Betriebssystem sieht dabei das ganze etwas anders aus. Ein einfaches Eingeben des Passworts sollte jedoch auf allen Betriebssystemen nicht funktionieren, was ihr auch an der Fehlermeldung oben sehen könnt. Die Registrierung durch den Browser ist eine valide Option, jedoch bietet das Erstellen eines Personal Access Tokens, den breitesten Einsatz. Wir erläutern nun also zunächst, wie das Token erstellt werden kann.

### Personal Access Token {#personalaccesstoken}

Das besondere an diesem Token ist, dass es an einen Nutzer gebunden ist. Das heißt, es wird diesselben Zugriffsrechte wie der User haben (bei Zugriff auf Content oder Spaces). Er kann auf verschiedenen Geräten genutzt werden, um mit <span style="color: darkred;">**GitHub**</span> zu kommunizieren. Um einen PAT zu erstellen, müsst ihr euch erstmal auf <span style="color: darkred;">**GitHub**</span> einloggen, falls ihr es gerade nicht sowieso seid. Dann navigiert ihr zu **Settings** und klickt in der linken Spalte ganz unten **Developer Settings** an. Hier geht ihr dann auf **Personal Access Token** und generiert ein neues Token. 
...
Nun kommt ein wichtiger Part: Ihr wählt Scopes aus und bestimmt so für was euer Token alles benutzt werden kann. Für unsere Zwecke reicht es, wenn ihr *repo* auswählt. Jetzt klickt ihr auf **Generate new Token**. Hier wählen wir erstmal die *Classic* Version, die auf jeden Fall stabil arbeitet. Nun seht ihr euer Token und speichert es in einem nur für euch einsichtbaren Dokument ab. Dieser Schritt ist wichtig, da ihr das Token nicht mehr auf <span style="color: darkred;">**GitHub**</span> ansehen könnt, wenn ihr einmal die Seite geschlossen habt. 

Den erstellten Token können wir jetzt im Terminal als Passwort nutzen oder in dem Pop-up angeben. Dann sollte bei dem Befehl auch kein Fehler mehr auftreten.
...
Anschließend wird viel Code angezeigt, der die angestrebten Aufgaben ausführt. Wenn ihr nun zurück auf das Browser-Fenster wechselt und euer Repository aufruft, sollten die Dateien, die ihr lokal erstellt hattet, angezeigt werden. 
...
In den *Settings* könnt ihr links *Collaborators* auswählen. Unten im Bild erscheint dann der grüne Button *Add people*. Personen, die mit euch am Projekt arbeiten sollen, könnt ihr nun via *username*, *full name* oder *e-mail* hinzufügen. 
