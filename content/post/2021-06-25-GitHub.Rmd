...
#### Fall 1: Neues `repository` auf `GitHub` erstellen
...
#### Fall 2: Lokal existierendes `repository` auf `GitHub` laden

Jetzt seht ihr nochmal, was ihr machen müsst, falls ihr euer lokal bereits existierendes `repository` auf GitHub laden wollt, um es mit anderen Personen zu teilen. Beispielsweise könnt ihr das `repository`, was wir beispielhaft für das Statistik-Praktikum erstellt haben, auf `GitHub` laden, damit eure Kommiliton:innen darauf zugreifen können.

Zunächst müsst ihr auf `GitHub` ein neues `repository` erstellen. Diesem gebt ihr am Besten den gleichen Namen wie eurem lokalen Ordner, also bei uns hieße es "praktikum":

*Screenshot, wo man draufklicken muss*

Achtet hierbei darauf, dass ihr das neue `repository` als öffentlich zugänglich - also "public" - erstellt.

Als nächstes müsst ihr den Link kopieren, der ganz oben in dem blauen `Quick Setup` Kasten zu sehen ist. Diesen brauchen wir gleich wieder in unserem Terminal.

Im Terminal geben wir den Befehl
```
git remote add origin [copied web address]
```
ein und fügen anstelle der eckigen Klammer den soeben kopierten Link unseres respositories ein.

Als letztes müsstet ihr noch
```
git push origin master
```
eingeben und euer lokales Projekt wurde erfolgreich auf `GitHub` hochgeladen! Wenn ihr die Seite von `GitHub` jetzt refresht, sollten eure Dateien zu sehen sein.
...
Als nächstes müsstet ihr euch eine lokale Version des bereits bestehenden Projekts auf euren Rechner herunterladen. Dazu wechseln wir wieder kurz in das **Terminal** des Computers.
...
git init
```
eingeben. Jetzt wurde der Ordner zu einem `git`-Ordner initialisiert und kann mit dem Projekt verknüpft werden.

Um das Projekt jetzt von `GitHub` in diesen Order "praktikum" zu laden, benötigt ihr wieder die URL des Projekts (siehe oben). In das Terminal geben wir nun den Befehl
```
git clone DieURL ###entsteht dadurch Bezug?
```
