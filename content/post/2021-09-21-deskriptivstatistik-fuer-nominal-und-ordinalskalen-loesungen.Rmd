...
Die Variable `prok2` enthält die Angaben über das Ausmaß, in dem Sie und Ihre Kommilitonen/-innen sich zum Zeitpunkt des Ausfüllens "müde" fühlten.  
...
```{r}
table(fb21$prok2)
prop.table(table(fb21$prok2))
```

Der Modus der Variable "prok2" ("müde") beträgt 3, d.h. die Antwortkategorie '3' wurde am häufigsten genannt (*n* = `r max(table(fb21$prok2))` bzw. `r round(max(prop.table(table(fb21$prok2)))*100,2)`%).
...
```{r}
quantile(fb21$prok2, c(.25,.5,.75), na.rm=T)
```

Der Median beträgt `r quantile(fb21$prok2, .5, na.rm=T)`. Das 1. und 3. Quartil betragen `r quantile(fb21$prok2, .25, na.rm=T)` bzw. `r quantile(fb21$prok2, .75, na.rm=T)`. Folglich beträgt der Interquartilsbereich (IQB) `r  quantile(fb21$prok2, .75, na.rm=T)-quantile(fb21$prok2, .25, na.rm=T)`.
