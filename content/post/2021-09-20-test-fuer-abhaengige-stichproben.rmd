...
Ein Blick auf die Messwertpaare lässt bereits erkennen, dass die Stichproben (also die Messwerte in den beiden experimentellen Bedingungen) voneinander abhängig sind. Die Geschwisterpaare ähneln sich hinsichtlich ihrer kooperativen Verhaltenstendenzen. Auch inhaltlich sind sie von einander abhängig, da die meisten Geschwister mit einander verwandt sind, also ähnliche Gene aufweisen, und in der Regel im gleichen Heim aufwachsen und somit gleiche/sehr ähnliche Umwelteinflüsse genießen. 

Für unsere Analysen ist es jedoch unwichtig, welche Ursachen diese Ähnlichkeiten haben. Entscheidend
ist, dass es sich um Faktoren handelt, die sowohl einen Teil der Varianz in der ersten Gruppe als auch einen Teil der Varianz in der zweiten Gruppe erzeugen.
...
Wie immer beginnen wir mit der deskriptiven Analyse unserer Daten.
...
par(mfrow=c(1,1)) #Zuruecksetzen des Plotfensters, zuvor hatten wir "dev.off()" kennengelernt
```
...
Wir verwenden wir die Funktion `t.test`. Diesmal müssen wir allerdings die beiden Gruppen einzeln der Funktion übergeben. Dies geschieht über die Argumente `x` und `y`. Das Argument `paired = T` führt dazu, dass der _t_-Test für abhängige (gepaarte) Stichproben durchgeführt wird.
...
$\Rightarrow$ der standardisierte Populationseffekt beträgt $d_2''$ = 1.46 und ist laut Konventionen groß. 
...
wilcox.test(x = dataKooperation[, "Juenger"], 
            y  = dataKooperation[, "Aelter"], # die beiden abhaengigen Gruppen
            paired = T,      # Stichproben sind abhaengig
            alternative = "greater", # gerichtete Hypothese
            conf.level = .95)                 # alpha = .05
```

_V_ = 4, _p_ < .01 $\rightarrow$ H0 wird verworfen, H1 wird angenommen.
...
Es wurde an Geschwisterpaaren untersucht, ob jüngere Geschwister kooperativer sind als ältere Geschwister. Zunächst findet sich deskriptiv ein Unterschied: Jüngere Geschwister weisen einen durchschnittlichen Wert von 0.43 (_SD_ = 0.12) auf, während die älteren Geschwister einen Wert von 0.32 (_SD_ = 0.07) aufweisen. Da die Differenzen nicht normalverteilt waren, wurde ein Wilcoxon-Test für abhängige Stichproben durchgeführt. Der Unterschied wurde bei einem Signifikanzniveau von alpha = .05 signifikant (_V_ = 44, _p_ < .01). Somit wird die Nullhypothese verworfen und die Alternativhypothese angenommen:  Jüngere Geschwister sind kooperativer als ihre älteren Geschwister.


### 5.2 Vergleich _t_-Test und Wilcoxon-Test
Der _t_-Test und der Wilcoxon-Test haben unterschiedliche Annahmen. Die des _t_-Tests sind strenger. Sind Annahmen verletzt, so kann es fälschlicherweise zu signifikanten Ergebnissen kommen. In unserem Beispiel ist es nun so, dass der _t_-Test _p_<0.001 und der Wilcoxon-Test _p_<0.01 sich hinsichtlich der p-Werte drastisch unterscheidet. Dies kann sich auf die Power der Tests auswirken. Dazu erfahren Sie in den Aufgaben zu Simulation und Poweranalysen am Ende des Semesters mehr!
