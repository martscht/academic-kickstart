...
lastmod: '2021-12-15 12:00:12 CEST'
featured: no
header:
...
Der Effekt der Zeit war signifikant negativ (was wir durch Betrachten der `t`-Werte und Vergleichen mit dem kritischen Wert $|t|> 1.96$ erkennen). 
...
Im Ergebnis sehen wir, dass Alter einen positiven Effekt auf das Psychische Wohlbefinden hat. Mit einem Jahr höherem Alter ändert sich das Niveau des Wohlbefindens um `r round(fixef(PWB.Age)["Age"],3)` Standardabweichungen. Das sieht numerisch wenig aus, bedeutet aber z. B. schon zwischen Zwanzigjährigen und Vierzigjährigen einen erwarteten Unterschied von `r round(20*fixef(PWB.Age)["Age"],3)` Standardabweichungen.
...
Für dieses Modell zentrieren wie auch die Zeit-Variable. Deren Nullpunkt ist zwar ohne Zentrierung anschaulicher, aber für die Interpretation des Moderatoreffekts sollten die beteiligten Prädiktoren zentriert sein. In den festen Effekten sehen wir (durch Betrachten der `t`-Werte und Vergleichen mit dem kritischen Wert $|t|> 1.96$), dass
