...
example1
```
Side note: if you want to learn more about creating data frames, have a look at our `R` [crash course](/post/r-crash-kurs), starting from the block "Daten".
...
To get a more in-depth overview of descriptive statistics, check out the tutorials on it on `pandaR`! There is one for [categorical and ordinally scaled variables](/post/deskriptiv-nominal/), as well as for [metric variables](/post/deskriptiv-intervall/).
...
For a more detailed description of how to investigate differences between two groups, check out the tutorial on [tests for independent groups](/post/gruppenvergleiche-unabhaengig/) or [this one for dependent groups](/post/gruppenvergleiche-abhaengig/). To get more information on the basics of distributions and confidence intervals, check out [this tutorial](/post/tests-und-konfidenzintervalle/).
...
### Correlation
We can start our analysis by checking whether participants' working memory correlates with recognition performance. For a more detailed tutorial on [correlation](/post/korrelation/), check out `pandaR`.
...
A more detailed tutorial on regression analysis can also be found on `pandaR`, where there's a special [tutorial](/post/regression/) on it.
