...
lastmod: '2023-05-22 12:00:12 CEST'
featured: no
header:
...
knitr::purl("2021-05-10_reg3.rmd", documentation = 0) #Legt fest das Purl nur die Chunkinhalte(0) in ein R-Skript umwandelt!; NAME DES RMD MUSS EINGESTELLT WERDEN!
file.rename("2021-05-10_reg3.R","PsyBSc7_R_Files/06_reg3.R") #NAME DER R-DATEI MUSS ANGEPASST WERDEN AN RMD-NAMEN!UNTERORDNER SOLLTE MIT KURS ÜBEREINSTIMMEN! (Siehe Ordner in /post)
```
...
* [Korrekte Spezifikation des Modells](#Theoretisch)
* [Messfehlerfreiheit der unabhängigen Variablen](#Theoretisch)
* [Unabhängigkeit der Residuen](#Theoretisch)
...
Anmerkung: In der Literatur werden oft auch unstandardisierte Regressionkoeffizienten als $\beta$s bezeichnet, sodass darauf stets zu achten ist - siehe bspw. [Appendix A](#AppendixA)).

An der standardisierten Lösung fällt auf, dass das Interzept $\beta_0$ als `NA` anezeigt wird. Dies liegt daran, dass beim Standardisieren die Mittelwerte aller Variablen auf $0$ und die Standardabweichungen auf $1$ gesetzt werden. So auch beim Kriterium, der Leseleistung. Damit beträgt der Mittelwert des Kriteriums und der Mittelwert aller Prädiktoren jeweils $0$. Das Interzept gibt an, welchen Wert das Kriterium hat, wenn alle Prädiktoren den Wert $0$ aufweisen. Somit muss das Interzept hier genau $0$ betragen - also in der Regressionsgleichung nicht vorhanden sein. Weil jeder geschätzte Parameter der Regressionsgleichung einen Freiheitsgrad **kostet**, wird der Parameter besser einfach nicht mitgeschätzt, also als `NA` angegeben.
...
<details><summary>**Standardisierte und Studentisierte Residuen**</summary>
...
## R-Skript
Den gesamten `R`-Code, der in dieser Sitzung genutzt wird, können Sie [`r fontawesome::fa("download")` hier herunterladen](/post/PsyBSc7_R_Files/06_reg3.R).
