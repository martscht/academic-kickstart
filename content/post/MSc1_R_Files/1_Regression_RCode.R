...
# Q-Q-Plot (vs. Normalverteilung)
qqPlot(our_model, pch = 16, distribution = "norm")

# Kolmogorov Smirnov Test (vs. Normalverteilung)
ks.test(x = res, y = "pnorm")

# Moderierte Regression
quad_int_model <- lm(reading ~ 1 + female*IQ   + I(IQ^2), data = Schulleistungen) #quadratische und Interaktionseffekte in das Modell aufnehmen
...
MD[MD > qchisq(p = .01, lower.tail = F, df = 2)] # Mahalanobiswerte > krit. Wert      # Mahalanobiswerte > krit. Wert
...
# Q-Q-Plot (vs. Chi^2(df = 2) Verteilung)
