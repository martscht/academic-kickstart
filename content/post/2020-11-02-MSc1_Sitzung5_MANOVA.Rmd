...
Wir erkennen, dass die Summary komplett identisch ist, zu der ersten ANOVA im zuvor generierten `summary.aov`-Objekt - nämlich der Post-Hoc ANOVA der Lebenszufriedenheit (`LZ`). Weitere Informationen zu ANOVAs können Sie [in den Unterlagen aus dem Bachelor](/lehre/#bsc7) finden. 
...
Noch besser sieht man es vielleicht mit einem Q-Q-Plot. Wir verwenden hier wieder die Funktion aus dem `car`-Paket, wie in der Sitzung zur [Regression](/post/regression-und-ausreisserdiagnostik).
...
## R-Skript
Den gesamten `R`-Code, der in dieser Sitzung genutzt wird, können Sie [`r fontawesome::fa("download")` hier herunterladen](/post/MSc1_R_Files/5_MANOVA_RCode.R).
...
<details><summary>**R-Code zu den Grafiken**</summary>
...
<details><summary>**MANOVA (und ANOVA) mit Messwiederholung**</summary>
