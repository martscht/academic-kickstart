...
Wie angekündigt finden Sie im Folgenden nun nochmal Aufgaben, die sich auf die Tutorials 5 bis 10 beziehen. Laden sie dafür den fb22 Datensatz herunter. load(url('https://pandar.netlify.app/post/fb22.rda'))
...
* Geben Sie zudem das 99%-ige Konfidenzintervall an.
...
fb22$geschl <- factor(fb22$geschl,levels = 1:3, labels = c("weiblich", "männlich", "anderes"))
```
...
```{r}
t.test(fb22_frauen$gewis, mu = 3.73, alternative = "greater", conf.level = .99) #mit conf.level = .99 wird uns direkt auch das gewünschte Konfidenzintervall ausgegeben
```
