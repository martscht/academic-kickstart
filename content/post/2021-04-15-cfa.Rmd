...
lastmod: '2021-04-21T14:30:02+02:00'
featured: no
header:
...
Wie Sie in der Theorie-Sitzung gesehen haben, sind die einzelnen Messgleichungen (wobei in der Theorie-Sitzung ggf. die vollständige Matrixnotation der Faktorladungen verwendet wurde, bei diesen also ggf. zwei Indizes für jeweils die Zeilen und Spalten der $\Lambda$-Matrix stehen; dies würde die Sache hier allerdings unnötig kompliziert machen, weshalb wir der Einfachheit halber darauf verzichten):
...
\begin{align}
  rel(Q7) &= \frac{\lambda_7^2 var(GC)}{var(Q7)} \\
  &= \frac{\lambda_7^2 var(GC)}{\lambda_7^2 var(GC) + var(\delta_7)} \\
  &= 1 - \frac{var(\delta_7)}{\lambda_7^2 var(GC) + var(\delta_7)}
\end{align}
