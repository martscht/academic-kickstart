...
lastmod: '2021-04-15T14:30:02+02:00'
featured: no
header:
  image: "/header/FEII_Sitzung3.jpg"
  caption: "[Courtesy of pxhere](https://pxhere.com/en/photo/478836)"
---

```{r, echo = FALSE, results = 'hide', message=FALSE, warning=FALSE}
load('./conspiracy_cfa.rda')
load('./stat_test.rda')
...
[In der ersten Sitzung](/post/lavaan-intro) hatten wir nur mit `~`, `~1` und `~~` für Regressionen, Intercepts und Residualvarianzen gearbeitet. Heute kommen Faktorladungen dazu.

Wie schon für die letzten beiden Sitzungen, können Sie das komplette R-Skript [`r fontawesome::fa("download")` hier herunterladen](/post/FEII_cfa.R).
...
Alternative können Sie den Datensatz natürlich auch für den lokalen Gebrauch [`r fontawesome::fa("download")` hier herunterladen](/post/conspiracy_cfa.rda).
...
![](/post/cfa1.png){width=33%}

### Modellsyntax

Wie wir es in der ersten Sitzung für die Regression gesehen haben, können wir dieses Pfaddiagramm in `lavaan` Modellsyntax übersetzen. Zunächst haben wir eine latente Variable, die wir nach Belieben benennen können. Weil es sich anbietet, nenne ich sie hier `GC` für "global conspiracy". Diese latente Variable wirkt sich auf drei manifesten Variablen aus. Bei diesen Variablen habe ich nicht die gleichen Freiheiten in der Benennung, weil es sich hier um manifeste Variablen handelt, die im Datensatz bereits existieren. Daher muss ich hier auch die Namen verwenden, die im Datensatz für die Variablen genutzt werden. Mit dem `=~` wird die Beziehung zwischen einer latenten Variable (links) und beliebig vielen manifesten Variablen (rechts) dargestellt. Diese Beziehung wird allgemein in Form von Faktorladungen quantifiziert. Mehrere manifeste Variablen, die der gleichen latenten Variable zugeordnet werden, können mit einem `+` verbunden werden. Für die Faktorstruktur sieht der Modellsyntax-Abschnitt also so aus:
...
Zunächst wissen Sie aus der Theorie-Sitzung, dass die Anzahl bekannter Informationen sich einfach als $s = \frac{p(p + 1)}{2}$ berechnen lässt. Dabei ist $s$ die Anzahl bekannter Informationen und $p$ die Anzahl von manifesten Variablen im Modell. In unserem Fall also
...
Auch hier ist die Festlegung auf `1` üblich, weil diese Zahl viele folgenden Berechnungen dramatisch vereinfacht. Von der puren Anwendung in `lavaan` her, können Sie aber jeder beliebige Zahl vor das `*` schreiben.
...
Wie schon [bei der Regression in Sitzung 1](/post/lavaan-intro) wird mit dem `.` vor dem Variablennamen deutlich gemacht, dass diese Variable in irgendeiner Form im Modell eine abhängige Variable ist. Das heißt, dass es sich bei der berichteten Varianz um eine *Residualvarianz* handelt. Für `GC` ist das nicht der Fall, weil - wie gerade besprochen - die latente Variable in der CFA die unabhängige Variable ist.
...
Die `summary` enthält nun den neuen Abschnitt `Covariances` der, wie Sie vermutlich erraten können, Kovarianzen zwischen den latenten Variablen enthält. Inspect erlaubt uns eine extrem detaillierte Untersuchung unserer Ergebnisse. Ein Blick in die Hilfe mit `?inspect` verrät, dass wir aber auch z.B. mit `what = 'cor.lv'` die "**cor**relations of **l**atent **v**ariables" anzeigen lassen können:
...
Wir sehen hier, dass die Cut-Off Werte, die für unsere Situation erzeugt wurden, sehr viel restriktiver sind als die generellen Leitlinien. Für den RMSEA ergibt sich ein Cut-Off von ca. `r round(cutoff$summary[4, 5], 2)`, statt der üblichen 0.05, für den SRMR sogar `r round(cutoff$summary[5, 5], 2)`. Anhand dieser spezifischen Cut-Off Werte muss unser Modell abgelehnt werden.
...
Wenn wir uns mit `inspect(fit_three, 'fitmeasures')` oder `fitmeasures(fit_three)` die Fitstatistiken dieses Modells ausgeben lassen, erhalten wir im direkten Vergleich mit dem Modell ohne Querladungung folgendes Bild:
...
Wie wir sehen, schneidet das Modell mit Querladung bei jeder Fitstatistik besser ab. Darüber hinaus muss das Modell auch anhand des $\chi^2$-Tests nicht mehr verworfen werden. Aber ist dieser Unterschied im Vergleich zum Modell ohne Querladung statistisch bedeutsam? Oder statistisch korrekt formuliert: müssen wir die Annahme, dass die Querladung exakt 0 ist (es also keine Querladung gibt) verwerfen? Für den direkten Vergleich zweier geschachtelter Modell haben Sie im Theorieabschnitt schon den *Likelihood-Ratio Test* kennengelernt.
...
Mit `(...)` wird in `lavaan` einem Parameter ein Label gegeben. Diese Labels können beliebige benannt sein, sie müssen nur mit einem Buchstaben beginnen. In der Zeile `test1 ~ (alp)*1` wird also das Intercept von `test1` angefordert und diesem Intercept der Name `alp` gegeben. In der nächsten Zeile machen wir für `test2` das Gleiche. Dadurch, dass wir hier den gleichen Namen nutzen werden die beiden Parameter zur Gleichheit gezwungen. In der Zusammenfassung tauchen diese Labels auch wieder auf, um zu verdeutlichen welche Parameter absichtlich gleich sind:
...
Wie schon im letzten Abschnitt müssen wir erneute mit Labels arbeiten, nur dass sie sich in diesem Fall auf die Residualvarianzen beziehen:
