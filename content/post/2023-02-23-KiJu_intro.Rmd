...
`R` und RStudio werden meist Synonym verwendet. Dabei gibt es eigentlich einen Unterschied. Während `R` eine Sprache ist, auf der also unsere ganze Syntax basieren wird, bietet RStudio eine benutzerfreundlichere Oberfläche. Weitere Infos zu den beiden Bestandteilen finden sich [hier](/post/r-crash-kurs/#R). Eine Beschreibung der Oberfläche kann man [hier](/post/r-crash-kurs/#RStudio) finden. Dort wird auch die Funktionalität der einzelnen Fenster beschrieben und die Wichtigkeit eines Skripts erläutert.
...
Weitere Informationen finden sich [hier](/post/r-crash-kurs/#Taschenrechner).
...
Hier könnt ihr klicken, um mehr zu [Funktionen](/post/r-crash-kurs/#Funktionen) und zur [Hilfe](/post/r-crash-kurs/#Hilfe) zu erfahren.
...
Eine detaillierte Aufbereitung zu Objekten und dem Environment ist an diesem [Ort](/post/r-crash-kurs/#Environment) zu finden.
...
Wir kommen jetzt noch einmal kurz auf die NA zu sprechen: Sie repräsentieren in `R` fehlende Werte. Einige Befehle zu fehlenden Werten finden sich [hier](/post/deskriptiv-nominal/#Fehlend).
...
Mehr zu der Erstellung von Skalenwerten und dem in diesem Zusammenhang unter Umständen erforderliche Rekodieren von Items findet sich [hier](/post/deskriptiv-intervall/#Rekodieren).
