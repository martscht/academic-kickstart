...
summary: ''
authors: [schultze]
lastmod: '2022-06-03T15:41:40+02:00'
featured: no
header:
...
knitr::purl("2022-05-30_nominalskalierte-praediktoren.Rmd", documentation = 0) #Legt fest das Purl nur die Chunkinhalte(0) in ein R-Skript umwandelt!; NAME DES RMD MUSS EINGESTELLT WERDEN!
file.rename("2022-05-30_nominalskalierte-praediktoren.R","PsyBSc7_R_Files/13_reg6.R") #NAME DER R-DATEI MUSS ANGEPASST WERDEN AN RMD-NAMEN!UNTERORDNER SOLLTE MIT KURS ÜBEREINSTIMMEN! (Siehe Ordner in /post)
```
...
pred_plot
```
Hierzu haben wir der Grafik die Datenpunkte des neuen Modells hinzugefügt und können diese gut mit der Vorhersage des alten Modells vergleichen (*wir mussten hier mit `tab$mod2` arbeiten und nicht einfach nur mit `mod2` in `geom_point` und `geom_line`, da sich die Daten geändert hatten und wir den Plot ansonsten nochmals hätten aufrufen müssen, was wir uns hier ersparen wollten*): Die beiden Linien haben jetzt einen Anstieg - dieser Anstieg entspricht dem Regressionsgewicht von `bribe`: $b_2 =$ `r round(coef(mod2)[3], 2)` und somit dem Unterschied zwischen den Vorhersagen für Gruppenbestechung und individuelle Bestechung:
...
$$  \hat{y}_m = (b_0 + b_2x_{2m}) + (b_1 + b_3x_{2m}) \cdot x_{1m} $$
...
## R-Skript
Den gesamten `R`-Code, der in dieser Sitzung genutzt wird, können Sie [`r fontawesome::fa("download")` hier herunterladen](/post/PsyBSc7_R_Files/13_reg6.R).
...
```{r complex_plot_additive, fig = TRUE, purl=FALSE}
scatter + 
...
```{r complex_plot_1interact, fig = TRUE, purl=FALSE}
# Modell
...
```{r complex_plot_fullinteract, fig = TRUE, purl=FALSE}
# Plot vorbereitung
tmp <- coef(mod13)
tab$intercept <- c(tmp[1], sum(tmp[c(1, 3)]), sum(tmp[1:2]), sum(tmp[c(1:3, 5)]))
