...
knitr::purl("2021-04-06_Wiederholung-von-Grundlagen.Rmd", documentation = 0) #Legt fest ob Purl nur die Chunkinhalte(0), auch die Chunkeinstellungen (1) oder direkt das Ganze Skript in ein R-Skript umwandelt!; NAME DES RMD MUSS EINGESTELLT WERDEN!
file.rename("2021-04-06_Wiederholung-von-Grundlagen.R","PsyBSc7_R_Files/01_wiederholung-grundlagen.R") #NAME DER R-DATEI MUSS ANGEPASST WERDEN AN RMD-NAMEN!UNTERORDNER SOLLTE MIT KURS ÜBEREINSTIMMEN! (Siehe Ordner in /post)
```
...
Das Praktikum des Moduls PsyBSc 7 baut auf Grundlagen in `R` auf, die Sie im Modul PsyBSc2 erlernt haben. Damit wir alle vom gleichen Punkt aus in das Semester starten können, wiedehrolen wir an dieser Stelle einige Inhalte. Wenn Sie eigenständig eine umfassendere Auffrischung der Inhalte des letzten Semesters benötigen oder sich gerne auf die Datenanalyse in R einstimmen möchten, können Sie jederzeit über Pandar auf die Inhalte von [PsyBSc 2](/lehre/#bsc2) zurückgreifen. 
...
Wenn Sie einmal etwas in der Konsole ausgeführt haben, anstatt es im Skript zu hinterlegen, können Sie anhand der **History** nachvollziehen, welche Schritte Sie durchlaufen haben. In dieser können Sie auch mehrere Zeilen bzw. Schritte markieren und diese über den![](/post/tosource.png) button in Ihre Syntax einfügen.
...
## R-Skript
Den gesamten `R`-Code, der in dieser Sitzung genutzt wird, können Sie [`r fontawesome::fa("download")` hier herunterladen](/post/PsyBSc7_R_Files/01_wiederholung-grundlagen.R).
...
<details><summary>**`R`-Code mit ChatGPT**</summary>
...
Darüber hinaus, könnten wir die KI an dieser Stelle auch bitten, solche selbstgeschriebenen Funktionen zu verbessern. Z.B. die Anzahl der [Loops](/post/loops-und-funktionen) in unseren Skripten zu minimieren, damit die Auswertung schneller und effizienter läuft.
