...
Die kann den Gleichungen für die zweifaktorielle ANOVA abgeleitet werden. Der beobachtete Werte der i-ten Person aus Stufe j des 1. Faktors (Faktor A) und Stufe k des 2. Faktors (Faktor B) setzt sich wie folgt zusammen: 

$$Y_{ijk} := \mu + \alpha_j+\beta_k + \gamma_{jk} + \varepsilon_{ijk},$$

wobei $\mu$ der Mittelwert über alle Messungen ist, $\alpha_j$ die Abweichung wegen Faktor A auf der j-ten Stufe, $\beta_k$ die Abweichung wegen Faktor B auf der k-ten Stufe,  $\gamma_{jk}$ die Abweichung wegen der spezifischen Kombination des Faktor A auf der j-ten Stufe und Faktor B auf Stufe k, was auch die Interaktionen dieser Gruppenkombination genannt wird und  $\varepsilon_{ijk}$ beschreibt das Residuum, dass zufälligen Abweichungen in den Gruppen darstellt (dieses wird als normalverteilt und unabhängig mit gleicher Varianz über alle Gruppen angenommen!). Nun können wir die Gleichen von oben hierzu übersetzten, denn $\mu_{j \bullet}$ ist der Mittelwert der j-ten Stufe auf Faktor A - hier wird also über alle $\beta_k$ und $\gamma_{jk}$ gemittelt (der Mittelwert ist immer 0, da wir am Ende immer bei $\mu$ landen müssen!). Somit ist $\alpha_j$ nichts anderes als die Differenz zwischen $\mu_{j \bullet}$ und $\mu$, also: $\alpha_j:=\mu_{j \bullet}-\mu$, entsprechend könnten die Hypothesen auch jeweils für die Effektparameter $\alpha, \beta, \gamma$ formuliert werden!
...
Mit *Tukeys Honest Significant Difference* können, wie auch in der [letzten Sitzung](/post/einfaktorielle-anova/), alle möglichen Gruppenkombinationen verglichen werden. Wir müssen die `TuckeyHSD` Funktion auf ein `aov`-Objekt anwenden.
...
So ein `aov`-Objekt hat auch weitere Vorteile, da wir bspw. dem `ez1` Objekt nicht direkt die Fehlerquadratsumme entlocken können, welche bspw. dafür benötigt werden, um $\eta^2$ zu bestimmen. Um einem `aov`-Objekt ähnliche Infos zu entlocken, wird in der Regel die `summary` Funktion verwendet. Genauso können auch weitere Werte, wie bspw. die Residuen, diesem Objekt entnommen werden, was bspw. mit der `resid` Funktion geht, welche bereits auch in der letzten Sitzung zur [einfaktoriellen Varianzanalyse](/post/einfaktorielle-ANOVA) angesprochen wurden, um die Normalverteilung der Residuen zu untersuchen.
