---
title: Daten für die Quiz
date: '2021-10-15'
slug: msc5a-daten
categories:
  - MSc5a
tags:
  - Daten
  - Einführung
  - Regression
  - Multi-Level Analyse
  - Hierarchische Regression
  - Logistische Regression

subtitle: ''
summary: ''
authors: [nehler, irmer]
lastmod: '2021-10-14T10:58:21+02:00'
featured: no
header:
  image: "/header/FEI_Data.jpg"
  caption: "[Courtesy of pxhere](https://pxhere.com/en/photo/734854)"
projects: []
---


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, fig.align = "center")
```



## Block 1: Wiederholung und Multiple Regression {#Block1}
**ACHTUNG! DER DATENSATZ IST NOCH NICHT VERFÜGBAR UND WIRD MIT DEM QUIZ BEREIT GESTELLT!**
Wie verwenden den Datensatz `Behandlungsformen.rda`. Sie können den [`r fontawesome::fa("download")` Datensatz "Behandlungsformen.rda" hier herunterladen](/post/under-construction). Liegt der Datensatz bspw. auf dem Desktop, so müssen wir den Dateipfad dorthin legen und können dann den Datensatz laden (wir gehen hier davon aus, dass Ihr PC "Musterfrau" heißt)

```{r, eval = F}
load("C:/Users/Musterfrau/Desktop/Behandlungsformen.rda")
```

Genauso sind Sie in der Lage, den Datensatz direkt aus dem Internet zu laden. Hierzu brauchen Sie nur die URL und müssen `R` sagen, dass es sich bei dieser um eine URL handelt, indem Sie die Funktion `url` auf den Link anwenden. Der funktionierende Befehl sieht so aus (wobei die URL in Anführungszeichen geschrieben werden muss):

```{r,eval = F}
load(url("https://pandar.netlify.app/post/Behandlungsformen.rda"))
```



