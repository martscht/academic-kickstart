...
Die Variable `geschl` liegt numerisch vor, es treten die Werte 1 und 2 empirisch auf. Anhand des Kodierschemas ([<i class="fas fa-download"></i> Variablenübersicht](/post/variablen.pdf)) kann den Zahlen eine inhaltliche Bedeutung zugewiesen werden. Beispielsweise bedeutet der Wert 1 "weiblich". Diese *Label* werden nun im Faktor hinterlegt.
...
Knapp 6% Ihres Jahrgangs geben als Lieblingsfach "Diagnostik/Methoden" an! Vielleicht können wir Sie mit dem nächsten Thema begeistern. :-)
...
sum((fb20$lz - mean(fb20$lz, na.rm = TRUE))^2, na.rm = TRUE) / (nrow(fb20)-1)
```

Achtung! Wir benötigen für die Varianzberechnung `n` (s. Formel)! Wir nutzen hier `nrow(fb20)-1`, weil `nrow(fb20)` nicht das richtige n anzeigt (eine Person hat einen fehlenden Wert, daher die Anzahl an Zeilen minus der eine fehlende Wert = n)
...
```{r}
# Umrechnung der Varianzen
var(fb20$lz, na.rm = TRUE) * (117 - 1) / 117
```
